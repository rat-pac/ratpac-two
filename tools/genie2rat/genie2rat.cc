  ...
  for (int i = 0; i < num_events; i++) {
    ...
    while ((genieparticle = dynamic_cast<genie::GHepParticle *>(event_iter.Next()))) {
      ...
      if (genieparticle->Status() == genie::kIStInitialState) {
        ...
        ratparent->SetMomentum(TVector3(1000.0 * genieparticle->Px(), 1000.0 * genieparticle->Py(),
                                        1000.0 * genieparticle->Pz()));  // GENIE outputs momentum and energy in GeV
      ...
      if (genieparticle->Status() == genie::kIStStableFinalState) {
        ...
        ratparticle->SetMomentum(TVector3(1000.0 * genieparticle->Px(), 1000.0 * genieparticle->Py(),
                                          1000.0 * genieparticle->Pz()));  // GENIE outputs momentum and energy in GeV
...
void parse_command_line(int argc, char **argv) {
  ...
  static struct option opts[] = {{"input", 1, NULL, 'i'}, {"output", 1, NULL, 'o'}, {"num-events", 1, NULL, 'N'}};
  int option_index = 0;
