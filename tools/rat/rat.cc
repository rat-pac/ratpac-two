  ...
  while (c != -1) {
    switch (c) {
      ...
      case 'h': help();
        exit(0);
        break;
      case 'V':cout << "RAT " << RATVERSION << std::endl;
        exit(0);
        break;
      case 'd': debug_log = true;
        break;
      case 'l': logfilename = RAT::optarg;
        break;
      case 'n': OutROOTProc::run_num = atoi(RAT::optarg);
        break;
      case 'r':run = strtol(RAT::optarg, NULL, 10);
        if (run <= 0)
          Log::Die("-r [num] option requires run number to be greater than 0");
        else
          options.run = run;
        break;
      case 's': options.seed = atol(RAT::optarg);
        break;
      case 'b': RAT::DB::Get()->SetServer(RAT::optarg);
        break;
      case 'i': options.input_filename = RAT::optarg;
        break;
