...
class TestDB : public CxxTest::TestSuite {
 public:
  void setUp() { db = new DB(); };

  void tearDown() { delete db; };

  void testGetDefault() { TS_ASSERT_DIFFERS(DB::Get(), static_cast<DB *>(0)); };

  void testLoad() { TS_ASSERT_EQUALS(db->Load("test.ratdb"), 1); };

  void testLoadAll() {
    TS_ASSERT_EQUALS(db->LoadAll("."), 1);
    TS_ASSERT_DIFFERS(db->GetDefaultTable("MEDIA", "vacuum"), static_cast<DBTable *>(0));
    TS_ASSERT_DIFFERS(db->GetDefaultTable("TEST", "foo"), static_cast<DBTable *>(0));
    TS_ASSERT_EQUALS(db->GetDefaultTable("NOEXIST", ""), static_cast<DBTable *>(0));
  };
  ...
  void testNoExistUserSet() {
    ...
    TS_ASSERT_THROWS_EQUALS(ltest0->GetI("myint"), DBNotFoundError & r, r, DBNotFoundError("TEST", "", "myint"));
    TS_ASSERT_THROWS_EQUALS(ltest0->GetS("foo"), DBNotFoundError & r, r, DBNotFoundError("TEST", "", "foo"));
