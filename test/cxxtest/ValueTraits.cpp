...
char *bytesToString(const unsigned char *bytes, unsigned numBytes, unsigned maxBytes, char *s) {
  ...
  s = copyString(s, "{ ");
  for (unsigned i = 0; i < numBytes; ++i, ++bytes) s = copyString(copyString(s, byteToHex(*bytes)), " ");
...
void ValueTraits<const double>::normalNumber(double t) {
  ...
  s = copyString(s, ".");
  for (unsigned i = 0; i < DIGITS_ON_RIGHT; ++i) s = numberToString((unsigned)(t *= BASE) % BASE, s);
}
