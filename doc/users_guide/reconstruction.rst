...
Parameters
''''''''''
No parameters are required though a position reconstruction should be run before and a light speed is needed
to calculate time residuals.  Several useful parameters can be set in macro, which allows the processor to be
run multiple times with different settings in a single macro.  Detailed implementations are illustrated in
...
``numer_time_resid_low``    ``double``                  Lower cut on time residuals in ns.  Used for ``ratio``.
``numer_time_resid_up``     ``double``                  Upper cut on time residuals in ns.  Used for ``ratio``.
...
* classifier result - ``mean`` is the mean time within a specified time window
* classifier result - ``stddev`` is the unbiased standard deviation of times within a specified time window
* classifier result - ``skewness`` is the standardized unbiased skewness of times within a specified time window
* classifier result - ``kurtosis`` is the standardized unbiased excess kurtosis of times within a specified time window
* classifier result - ``num_PMT`` is the number of PMTs used in a reconstruction
* classifier result - ``num_PMT_numer`` is the number of PMTs used in a reconstruction
* classifier result - ``num_PMT_denom`` is the number of PMTs used in a reconstruction
* classifier result - ``time_resid_low`` is the earliest time residual that passes the lower time residual cut
* classifier result - ``time_resid_up`` is the latest time residual that passes the upper time residual cut
