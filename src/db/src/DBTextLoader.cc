...
Tokenizer::Tokenizer(std::string thedoc, std::string _filename)
    : doc(thedoc), filename(_filename) {
  len = doc.size();
...
std::string PickFile(std::string name, std::string enclosing_file) {
  ...
  if (enclosing_file != "") {
    newname =
        std::string(gSystem->DirName(enclosing_file.c_str())) + "/" + name;
...
Tokenizer::Type TokenizerStack::Next() {
  ...
  // Keep popping tokenizer until we get to something, or give up at the last
  // one
...
DBTable *Parser::Next() {
  ...
  while (toktype != Tokenizer::TYPE_EOF && state != STATE_END) {
    switch (state) {
    case STATE_START:
      if (toktype == Tokenizer::TYPE_SYMBOL && tokenizer.Token() == "{")
        state = STATE_TABLE;
      else if (toktype == Tokenizer::TYPE_SYMBOL && tokenizer.Token() == "#") {
        // Eat include token
        toktype = tokenizer.Next();
        if (toktype != Tokenizer::TYPE_IDENTIFIER &&
            tokenizer.Token() != "include")
        ...
        string filename =
            PickFile(tokenizer.Token(), tokenizer.GetCurrentFilename());
        string contents;
    ...
    case STATE_VAL_SEP:
      if (Tokenizer::IsValType(toktype)) {
        ...
        case Tokenizer::TYPE_STRING:
          tbl->Set(identifier, tokenizer.Token());
          break;
        case Tokenizer::TYPE_INTEGER:
          tbl->Set(identifier, tokenizer.AsInt());
          break;
        case Tokenizer::TYPE_DOUBLE:
          tbl->Set(identifier, tokenizer.AsDouble());
          break;
        default:
    ...
    case STATE_ARRAY:
      if (Tokenizer::IsValType(toktype)) {
        ...
        switch (toktype) {
        case Tokenizer::TYPE_STRING:
          string_array.push_back(tokenizer.Token());
          break;
        case Tokenizer::TYPE_INTEGER:
          integer_array.push_back(tokenizer.AsInt());
          break;
        case Tokenizer::TYPE_DOUBLE:
          double_array.push_back(tokenizer.AsDouble());
          break;
        default:
          assert(0); // Should never get here
        }
        ...
        switch (array_type) {
        case Tokenizer::TYPE_STRING:
          tbl->Set(identifier, string_array);
          break;
        case Tokenizer::TYPE_INTEGER:
          tbl->Set(identifier, integer_array);
          break;
        case Tokenizer::TYPE_DOUBLE:
          tbl->Set(identifier, double_array);
          break;
        ...
        tokenizer.RaiseError(string("Unexpected symbol in array: ") +
                             tokenizer.Token());
      ...
      if (toktype == Tokenizer::TYPE_SYMBOL && tokenizer.Token() == "]") {
        ...
        if (toktype == Tokenizer::TYPE_INTEGER) {
          ...
          for (int i = 0; i < repeat; i++) {
            switch (array_type) {
            case Tokenizer::TYPE_STRING:
              string_array.push_back(string_array.back());
              break;
            case Tokenizer::TYPE_INTEGER:
              integer_array.push_back(integer_array.back());
              break;
            case Tokenizer::TYPE_DOUBLE:
              double_array.push_back(double_array.back());
              ;
              break;
            default:
              assert(0); // Should never get here
          ...
          tokenizer.RaiseError(
              "Repetition count for value must be positive integer.");
  ...
  try {
    ...
    while (table != 0) {
      ...
      if (table->GetFieldType("run_range") == DBTable::INTEGER_ARRAY &&
        ...
        if (valid_begin[0] == 0 && valid_begin[1] == 0 && valid_end[0] == 0 &&
            valid_end[1] == 0) {
          ...
          table->SetRunRange(run_range[0], run_range[1]);
        } else if (valid_begin[0] == -1 && valid_begin[1] == -1 &&
                   valid_end[0] == -1 && valid_end[1] == -1) {
