...
std::vector<double> unpack_double(const std::string &contents) {
  ...
  for (unsigned i = 0; i < len; i++) {
    ...
    // ROOT defines R__BYTESWAP when platform is little-endian.  Since that is
    // server byte-ordering we swap when it is NOT defined
#ifndef R__BYTESWAP
...
DBTable *DB::FindTable(std::string tblname, std::string index, int runNumber) {
  ...
  // tracked before in the first place, but keep commented for historical
  reasons.
  ...
  while (tablesFromServer.size() > 0 && serverTableBytes + newTableBytes >
  maxServerBytes) { std::pair<DBTableKey, bool> forDeletion =
  tablesFromServer.front(); tablesFromServer.pop_front(); int deleteSize =
