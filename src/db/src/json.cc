...
std::vector<std::string> Value::getMembers() const {
  ...
  size_t i = 0;
  for (TObject::iterator pair = data.object->begin();
       pair != data.object->end(); ++pair) {
    keys[i++] = pair->first;
  }
  return keys;
}
...
void Value::wrongType(Type actual, Type requested) {
  ...
  pretty << "JSON Value of type " << prettyType(actual) << " is not type "
         << prettyType(requested);
  throw std::runtime_error(pretty.str());
}

parser_error::parser_error(const int line_, const int pos_, std::string desc_)
    : line(line_), pos(pos_), desc(desc_) {
...
const char *parser_error::what() const throw() { return pretty.c_str(); }
...
bool Reader::getValue(Value &result) {
  ...
  throw parser_error(line, cur - lastbr,
                     "Should never reach here. Probably hardware error.");
}
    ...
    case 'u': // non-json explicit unsigned
      *cur = '\0';
      cur++;
      {
        ...
        if (end != cur - 1)
          throw parser_error(line, cur - lastbr, "Malformed integer");
        return v;
      }
    case 'd': // non-json explicit real OR strange exponential
      *cur = '\0';
      cur++;
      {
        ...
        if (end != cur - 1)
          throw parser_error(line, cur - lastbr, "Malformed real");
        return v;
      }
    ...
    default: { // any other character is end of number
      ...
      *cur = '\0';
      ...
      if (real || exp) {
        ...
        // sponge");
        if (end != cur)
          throw parser_error(line, cur - lastbr, "Malformed real: " + (next));
      } else {
        errno = 0;
        char *end;
...
Value Reader::readObject() {
  ...
  for (;;) {
    ...
    case ':':
      ...
      if (!getValue(val)) {
        throw parser_error(line, cur - lastbr,
                           "EOF reached while parsing object");
      }
    ...
    case '\0':
      throw parser_error(line, cur - lastbr,
                         "Reached EOF while parsing object");
    default:
      if (keyfound) {
        throw parser_error(line, cur - lastbr,
                           "Unexpected character where value expected");
      }
      if (!key)
        key = cur;
      cur++;
...
Value Reader::readArray() {
  ...
  for (;;) {
    ...
    case ':': { // non-json value repetition
      cur++;
      Value reps;
      if (!getValue(reps) || reps.getType() != TINTEGER ||
          reps.getInteger() < 0) {
        throw parser_error(line, cur - lastbr,
                           "Array value repetition syntax error");
      }
    ...
    default:
      if (!getValue(next)) {
        throw parser_error(line, cur - lastbr,
                           "EOF reached while parsing array");
      }
      array.data.array->push_back(next);
    }
  }
...
std::string Writer::escapeString(std::string unescaped) {
  ...
  while (pos < len) {
    ...
    case '\t':
      ...
      break;
    default:
      if (unescaped[pos] < 0x20)
        throw parser_error(
            0, 0, "Arbitrary unicode escapes not yet supported"); // FIXME
    }
    pos++;
  }
  escaped << unescaped.substr(last, pos - last);
  ...
  if (escaped.find("\\") != std::string::npos) {
    ...
    while ((pos = escaped.find("\\", pos)) != std::string::npos) {
      ...
      case 'u':
        throw parser_error(
            line, cur - lastbr,
            "Arbitrary unicode escapes not yet supported"); // FIXME
      default:
        throw parser_error(line, cur - lastbr,
