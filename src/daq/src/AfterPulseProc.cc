...
/*
After-pulsing adapted from snoplus rat. Settings for after-pulsing are controlled using AFTERPULSEProc.ratdb. Parameters are as follows:
...
namespace RAT {
			...
			switch(fAPFlag){
				...
				case 1:{
					try{
						...
						std::cout << "Per model information missing from AFTERPULSEProc.ratdb, using default values storedin AFTERPULSEProc.ratdb." << "\n";
						APFraction = fDefAPFraction;
						APTime = fDefAPTime;
						APProb = fDefAPProb;
						PMTTime = fDefPMTTime;
				...
				case 2:{
					try{
						...
						std::cout << "Per individual PMT information missing from PMTINFO.ratdb, using default values storedin AFTERPULSEProc.ratdb." << "\n";
						APFraction = fDefAPFraction;
						APTime = fDefAPTime;
						APProb = fDefAPProb;
						PMTTime = fDefPMTTime;
	...
	double AfterPulseProc::CalculateAfterPulseTime(double APFraction, std::vector<double> APTime, std::vector<double> APProb){
		...
		double newtime = (randtime - APProb[(up-1)])*(APTime[up] - APTime[(up-1)])/(APProb[up] - APProb[(up-1)]) + APTime[(up-1)];
		return newtime;
	}
	...
	double AfterPulseProc::PickFrontEnd(double time, std::vector<double> PMTTime, std::vector<double> PMTProb, double CableDelay){//function for picking the front end time
		...
		for (size_t i = 1; i < PMTTime.size(); i++){
			if(rval <= PMTProbCumu[i]){
				return time + CableDelay + (rval - PMTProbCumu[i-1])*(PMTTime[i]-PMTTime[i-1])/(PMTProbCumu[i]-PMTProbCumu[i-1])+PMTTime[i-1];
			}
		}
	...
	void AfterPulseProc::DetectAfterPulse(DS::MC* mc, DS::PMTInfo* pmtinfo, const uint64_t event_time, std::vector<double> PMTTime, std::vector<double> PMTProb, double CableDelay){
		for(int iPMT = 0; iPMT < fPMTCount; iPMT++){ 
