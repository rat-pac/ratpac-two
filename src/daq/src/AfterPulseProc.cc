...
void AfterPulseProc::GenerateAfterPulses(DS::MC* mc, DS::PMTInfo* pmtinfo, const uint64_t eventTime) {
    //Afterpulses should only be generated from PMTs that have PEs on them, so use MCPMTs instead of looping over all PMTs
        ...
        for (int iPE = 0; iPE < nPE; iPE++) {           // Loop over PEs
            ...
            while (it != afterPulses.end()) {  // Loops over APs
                ...
                if (afterPulseTime > lowWindow && afterPulseTime < highWindow) {
                    ...
                    //Previously a bespoke routine to sample from the time distribution, now just use the standard method a la NoiseProc
                    //Is this double counting the smearing?
