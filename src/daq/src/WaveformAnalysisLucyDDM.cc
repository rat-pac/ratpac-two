...
void WaveformAnalysisLucyDDM::DoAnalysis(DS::DigitPMT* digitpmt, const std::vector<UShort_t>& digitWfm) {
  ...
  debug << "Hit finding took " << static_cast<long>(std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()) << " ms."
        << newline;
...
void WaveformAnalysisLucyDDM::RequestFFTSize(size_t size) {
  if (!fft || fft->GetSize() < size) {
    ...
    info << "Plan Creation took "
         << static_cast<long>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start)
                .count())
         << " ms." << newline;
  }
}
