...
Processor::Result TrueDAQProc::DSEvent(DS::Root *ds) {
    ...
    // Anything that has a positron and a neutron as the only MCParticles will be considered an IBD event.
    // Any other events will be treated as single events.
    // Truth information for "sub"-MC events is stored in the PathFit structure
    // This is a disgusting abuse of the data structure so I'm not gonna bother being very clean
    DS::MC *mc = ds->GetMC();
    ...
    //IBD variables. We always fill positron first so don't need to consider coming back to it
    bool foundPositron = false;
    bool foundNeutron = false;
    int neutronTrackIndex = -1;
    
    //"Singles" variables. We keep track of the number of MCParticles and the appropriate track/step indices for most
    //energetic point to use as vertex
    ...
    for (int imctrack = 0; imctrack < mc->GetMCTrackCount(); imctrack++) {
        //Ignore triggering on secondaries unless a decay is involved and always ignore opticalphotons
        ...
        //One day I'll update this to trigger on energy deposition just like we do on hits but for now
        //we'll be dumb and just trigger on the highest energy step associated with primary particles
        //IBD requires annoying checks to maintain order in event structre
        if (isSingle || (isIBD && !foundPositron && !foundNeutron && mctrack->GetPDGCode() == -11)) {
            ...
            for (int imctrackstep = 0; imctrackstep < mctrack->GetMCTrackStepCount(); imctrackstep++) {
                ...
                //If step is most energetic, set variables. If gamma, look at delta KE to see what Compton e- energy is
                //so that we don't have to descend the tracks
            ...
            if (isIBD) {
                foundPositron = true;
                //Alter index variable to get to the IBD neutron on the next loop if we've seen it
        ...
        //For IBD, store neutron track index if it comes before the positron. foundNeutron stays off since that's to indicate we're ready to build that "ev"
        if (isIBD && !foundPositron && !foundNeutron && mctrack->GetPDGCode() == 2112 && mctrack->GetParentID() == 0) {
            neutronTrackIndex = imctrack;
        }
        //For IBD, if we haven't found the positron track, then we continue looping until we do without triggering
        if (isIBD && !foundPositron) {
            continue;
        }
        //For "singles", we need to check all MCParticles before we can do the vertex
        ...
        TVector3 dir(0,0,0);
        //For IBD, if we've found the positron, now we take the neutron capture position (last step)
        if (isIBD && foundPositron && !foundNeutron && mctrack->GetPDGCode() == 2112 && mctrack->GetParentID() == 0) {
        ...
        //For "singles", if we have all the MCParticles, we know the most energetic step from a primary. Do same for IBD prompt positron
        if ((isSingle && mcParticleCount == mc->GetMCParticleCount()) || (isIBD && foundPositron && mctrack->GetPDGCode() == -11 && mctrack->GetParentID() == 0)) {
            ...
            //This assumes forward scattering if most energetic step is from gamma. We take the displacement vector from the previous indexed step since if
            //we lost all our energy in this step, we won't have a momentum and also this was the direction of travel as the energy was deposited,
            //since we save the step endpoints
            dir = (mc->GetMCTrack(mostEnergeticStepTrackIndex)->GetMCTrackStep(mostEnergeticStepStepIndex)->GetEndpoint()
                - mc->GetMCTrack(mostEnergeticStepTrackIndex)->GetMCTrackStep(mostEnergeticStepStepIndex-1)->GetEndpoint()).Unit();
        }
        ...
        if (tt >= lastTrigger + fTriggerWindow + fTriggerLockout) {
            ...
            for (int jmctrack = 0; jmctrack < mc->GetMCTrackCount(); jmctrack++) {
                ...
                bool allInWindow = initTime >= lastTrigger + fTriggerWindow + fTriggerLockout && initTime < tt + fTriggerWindow + fTriggerLockout
                                    && finalTime >= lastTrigger + fTriggerWindow + fTriggerLockout && finalTime < tt + fTriggerWindow + fTriggerLockout;
                bool allOutWindow = (initTime < lastTrigger + fTriggerWindow + fTriggerLockout && finalTime < lastTrigger + fTriggerWindow + fTriggerLockout)
                ...
                //Otherwise we have to step through the tracks and find the inwindow bits
                else {
                  for (int jmctrackstep = 0; jmctrackstep < mctrack2->GetMCTrackStepCount(); jmctrackstep++) {
                      ...
                      //If time of MCStep is after previous trigger window ends and before current trigger window ends, count as deposited in this event
                      if (mctrackstep2->GetGlobalTime() >= lastTrigger + fTriggerWindow + fTriggerLockout && mctrackstep2->GetGlobalTime() < tt + fTriggerWindow + fTriggerLockout) {
