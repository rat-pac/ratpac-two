...
#include "globals.hh"
//#include "G4LightIonQMDPhysics.hh" //G4 11.2V
...
MyShielding::MyShielding(G4int verbose, const G4String& n_model, const G4String& HadrPhysVariant,
                         G4bool useLightIonQMD) {
  ...
  if (verbose > 0) {
    G4cout << "<<< Geant4 Physics List simulation engine: Shielding" << HadrPhysVariant << G4endl;
    if (LEN_model == "LEND") {
      G4cout << "<<< LEND will be used for low energy neutron and gamma projectiles" << G4endl;
    } else {
      G4cout << "<<< (Note that Shielding" << HadrPhysVariant << " and Shielding" << HadrPhysVariant
             << "_HP are equivalent!)" << G4endl;
    }
  }
  ...
  if (LEN_model == "HP") {
    ...
    if (verbose > 0) {
      ...
      G4cout << "\"" << LEN_model << "\" is not valid for the low energy neutron model." << G4endl;
  ...
  if (HadrPhysVariant == "M") {
    ...
    hpc = new MyG4HadronPhysicsShielding("hInelastic My_Shielding", verbose, 9.5 * CLHEP::GeV, 9.9 * CLHEP::GeV);
  } else {
    hpc = new MyG4HadronPhysicsShielding("hInelastic My_Shielding", verbose,
  ...
  if (LEN_model == "LEND") {
    ...
    if (HadrPhysVariant == "M") {
      ...
      // between the string model and cascade model, therefore the recommended
      // values from G4HadronicParameters are intentionally not used.
      hpc = new MyG4HadronPhysicsShieldingLEND("hInelastic My_ShieldingLEND", verbose, 9.5 * CLHEP::GeV,
                                               9.9 * CLHEP::GeV);
    } else {
      hpc = new MyG4HadronPhysicsShieldingLEND("hInelastic My_ShieldingLEND", verbose,
                                               G4HadronicParameters::Instance()->GetMinEnergyTransitionFTF_Cascade(),
                                               G4HadronicParameters::Instance()->GetMaxEnergyTransitionFTF_Cascade());
    }
  } else {
