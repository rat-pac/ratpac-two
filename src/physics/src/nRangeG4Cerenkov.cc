...
////////////////////////////////////////////////////////////////////////
//
// File:        nRangeG4Cerenkov.cc
// Description: Discrete Process -- Generation of Cerenkov Photons
// Version:     2.1
// Created:     1996-02-21
// Author:      Juliet Armstrong
// Updated:     2025-09-30 by Zach Larsen
// 		> updated the third conditonal statement in GetAverageNumberOfPhotons
// 		to iterate through the RINDEX and find regions satisfying
// 		the Cerenkov condition (n >= 1/beta)
// 		> this approach can now handle materials with anomalous dispersion
// 		2007-09-30 by Peter Gumplinger
...
nRangeG4Cerenkov::nRangeG4Cerenkov(const G4String& processName, G4ProcessType type)
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
nRangeG4Cerenkov::~nRangeG4Cerenkov() {
...
void nRangeG4Cerenkov::ProcessDescription(std::ostream& out) const {
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4bool nRangeG4Cerenkov::IsApplicable(const G4ParticleDefinition& aParticleType) {
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void nRangeG4Cerenkov::Initialise() {
  G4OpticalParameters* params = G4OpticalParameters::Instance();
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void nRangeG4Cerenkov::BuildPhysicsTable(const G4ParticleDefinition&) {
  ...
  for (std::size_t i = 0; i < numOfMaterials; ++i) {
    ...
    refractiveIndexValuesVectors->push_back(refractiveIndexValues);
    refractiveIndexEnergyVectors->push_back(refractiveIndexEnergy);
  }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4VParticleChange* nRangeG4Cerenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
  ...
  // see bugzilla 2555
  if (fNumPhotons <= 0 || !fStackingFlag || std::max(MeanNumberOfPhotons1, MeanNumberOfPhotons2) < 1e-15) {
    // return unchanged particle and no secondaries
    aParticleChange.SetNumberOfSecondaries(0);
    return pParticleChange;
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void nRangeG4Cerenkov::PreparePhysicsTable(const G4ParticleDefinition&) { Initialise(); }

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double nRangeG4Cerenkov::GetMeanFreePath(const G4Track&, G4double, G4ForceCondition*) { return 1.; }

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double nRangeG4Cerenkov::PostStepGetPhysicalInteractionLength(const G4Track& aTrack, G4double,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double nRangeG4Cerenkov::GetAverageNumberOfPhotons(const G4double charge, const G4double beta,
...
// Geant4-unit (millimeter) in the current medium.
{
  ...
  // otherwise if n(Pmin) > 1/Beta -- photons generated across all wavelengths
  ...
  // threshold
    ...
    // integrate each segment where n >= 1/Beta
    for (std::size_t i = 0; i < rindex_vals->size() - 1; ++i) {
      G4double E1 = (*rindex_energy)[i];
      G4double E2 = (*rindex_energy)[i + 1];
      G4double n1 = (*rindex_vals)[i];
      G4double n2 = (*rindex_vals)[i + 1];

      // Check if both points are within energy range and above the Cerenkov threshold
      if (E1 >= Pmin && E2 <= Pmax && n1 >= BetaInverse && n2 >= BetaInverse) {
        G4double dE = E2 - E1;
        validEnergyRange += dE;

        // Get the Cerenkov angle integrals for this segment
        G4double CAI1 = CerenkovAngleIntegrals->Value(E1);
        G4double CAI2 = CerenkovAngleIntegrals->Value(E2);
        totalCAI += (CAI2 - CAI1);
      }
      // Handle segments that partially satisfy the condition
      else if (E1 >= Pmin && E2 <= Pmax &&
               ((n1 >= BetaInverse && n2 < BetaInverse) || (n1 < BetaInverse && n2 >= BetaInverse))) {
        // Linear interpolation to find crossing point
        G4double energyCrossing = E1 + (E2 - E1) * (BetaInverse - n1) / (n2 - n1);

        if (n1 >= BetaInverse) {
          // Valid from E1 to crossing point
          G4double dE = energyCrossing - E1;
          validEnergyRange += dE;
          G4double CAI1 = CerenkovAngleIntegrals->Value(E1);
          G4double CAIcrossing = CerenkovAngleIntegrals->Value(energyCrossing);
          totalCAI += (CAIcrossing - CAI1);
        } else {
          // Valid from crossing point to E2
          G4double dE = E2 - energyCrossing;
          validEnergyRange += dE;
          G4double CAIcrossing = CerenkovAngleIntegrals->Value(energyCrossing);
          G4double CAI2 = CerenkovAngleIntegrals->Value(E2);
          totalCAI += (CAI2 - CAIcrossing);
        }

        if (verboseLevel > 1) {
          G4cout << "Partial segment with crossing at " << energyCrossing / eV << " eV" << G4endl;
        }
      }
    }
    ...
    if (verboseLevel > 1) {
      ...
      G4cout << "Total CAI: " << totalCAI << G4endl;
    }
  }
  return NumPhotons;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void nRangeG4Cerenkov::SetVerboseLevel(G4int verbose) {
