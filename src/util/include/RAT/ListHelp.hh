...
template <typename T> void deepdelete_vector(std::vector<T *> &vect) {
...
void deepcopy_vector(std::vector<T *> &dest, const std::vector<T *> &src) {
  ...
  for (unsigned i = 0; i < src.size(); i++) {
    if (src[i])
      dest[i] = dynamic_cast<T *>(
          src[i]->Clone()); // Use Clone() for proper polymorphism
    else
      dest[i] = 0;
  }
}
...
void endcopy_vector(std::vector<T *> &dest, const std::vector<T *> &src) {
  ...
  dest.resize(2);
  if (src.size() >= 2) { //
    dest[0] = dynamic_cast<T *>(
        src.front()->Clone()); // Use Clone() for proper polymorphism
