...
// The radix (i.e. base) for these conversions can be any value from base 2 to
// base 36 specifying any other radix causes std::invalid_argument to be thrown
...
// The width argument specifies the number of numerical digits to use in the
// result This is a minimum - if the value requires more digits then it will be
// wider than the width argument However, if it is smaller, then it will be
// extended to the specified width Then, the radix display prefix is added to
...
// If the radix is set to zero, the conversions deduce the radix from the string
// representation So, 0b prefix is binary, 0 prefix is octal, 0x is hex and
// <base># prefix is my hash format A non-zero radix should be used when the
// string value has no radix information and is non-decimal e.g. the hex value
// FEDCBA has no indication that it is hex, so specify radix 16 The radix must
