...
template <typename T>
static std::string util_simage(T i, unsigned radix, radix_display_t display,
...
// unsigned version
template <typename T>
static std::string util_uimage(T i, unsigned radix, radix_display_t display,
                               unsigned width) throw() {
  if (radix < 2 || radix > 36)
    throw std::invalid_argument("invalid radix value " + util_to_string(radix));
  // untangle all the options
...
std::string util_to_string(bool i, unsigned radix, radix_display_t display,
                           unsigned width) throw() {
  // use the char representation for bool
  return util_uimage<char>(i, radix, display, width);
}

std::string util_to_string(short i, unsigned radix, radix_display_t display,
                           unsigned width) throw() {
  return util_simage(i, radix, display, width);
}

std::string util_to_string(unsigned short i, unsigned radix,
                           radix_display_t display, unsigned width) throw() {
  return util_uimage(i, radix, display, width);
}

std::string util_to_string(int i, unsigned radix, radix_display_t display,
                           unsigned width) throw() {
  return util_simage(i, radix, display, width);
}

std::string util_to_string(unsigned i, unsigned radix, radix_display_t display,
                           unsigned width) throw() {
  return util_uimage(i, radix, display, width);
}

std::string util_to_string(long i, unsigned radix, radix_display_t display,
                           unsigned width) throw() {
  return util_simage(i, radix, display, width);
}

std::string util_to_string(unsigned long i, unsigned radix,
                           radix_display_t display, unsigned width) throw() {
  return util_uimage(i, radix, display, width);
}
...
std::string util_to_string(float f, real_display_t display, unsigned width,
...
std::string util_to_string(double f, real_display_t display, unsigned width,
                           unsigned precision) throw() {
  std::string format;
  switch (display) {
  case display_fixed:
      ...
      for (std::string::size_type j = i; j < str.size(); j++) {
        ...
        default:
          throw std::invalid_argument("invalid character in string " + str +
                                      " for radix " + util_to_string(radix));
          break;
        }
      }
    } else if (radix == 8) {
...
bool util_to_bool(const std::string &str, unsigned radix) throw() {
  return uvalue<unsigned char>(str, radix) != 0;
}

short util_to_short(const std::string &str, unsigned radix) throw() {
...
int util_to_int(const std::string &str, unsigned radix) throw() {
  return svalue<int>(str, radix);
}

unsigned int util_to_uint(const std::string &str, unsigned radix) throw() {
  return uvalue<unsigned>(str, radix);
}

long util_to_long(const std::string &str, unsigned radix) throw() {
...
std::string util_join(const std::vector<std::string> &str,
                      const std::string &joiner, const std::string &prefix,
