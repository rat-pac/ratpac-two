...
TrackingMessenger::TrackingMessenger() {
  ...
  FillPointContCmd->SetGuidance(
      "Enables/disables G4 track visualization via /vis commands.");
  FillPointContCmd->SetGuidance("This requires the RAT trajectories to "
                                "duplicate info, consuming more space.");
  FillPointContCmd->SetParameterName("on", true, false);
  FillPointContCmd->SetDefaultValue(false);

  storeParticleTrajCmd =
      new G4UIcmdWithAString("/tracking/storeParticleTrajectory", this);
  storeParticleTrajCmd->SetGuidance(
      "Store trajectory of a particle with given name");
  storeParticleTrajCmd->SetParameterName(
      "particleName", false); // required string: particle name

  discardParticleTrajCmd =
      new G4UIcmdWithAString("/tracking/discardParticleTrajectory", this);
  discardParticleTrajCmd->SetGuidance(
      "Discard trajectory of a particle with given name");
  discardParticleTrajCmd->SetParameterName(
      "particleName", false); // required string: particle name

  storeMuonTrajSpecialCmd =
      new G4UIcmdWithABool("/tracking/storeMuonTrajectorySpecial", this);
  ...
  storeOpticalTrackIDCmd =
      new G4UIcmdWithABool("/tracking/storeOpticalTrackID", this);
  storeOpticalTrackIDCmd->SetGuidance(
      "Store original parent track ID and creation step for each MC photon.");
  storeOpticalTrackIDCmd->SetParameterName("state", true, false);
  storeOpticalTrackIDCmd->SetDefaultValue(false);

  setMaxGlobalTimeCmd =
      new G4UIcmdWithADouble("/tracking/setMaxGlobalTime", this);
...
G4String TrackingMessenger::GetCurrentValue(G4UIcommand *command) {
  if (command == FillPointContCmd) {
    ...
    return setMaxGlobalTimeCmd->ConvertToString(
        GLG4SteppingAction::max_global_time);
  } else {
    Log::Die("TrackingMessenger sent unknown get command: " +
...
void TrackingMessenger::SetNewValue(G4UIcommand *command, G4String newValue) {
  if (command == FillPointContCmd) {
    ...
    if (state)
      detail << "Tracking: Storing abbreviated tracking information for muons"
             << newline;
    else
      detail
          << "Tracking: Not storing abbreviated tracking information for muons"
          << newline;
    ...
    if (val == 0.0)
      ...
      detail << "Tracking: Max global time for tracks set to " << val
             << newline;
    GLG4SteppingAction::max_global_time = val;
  } else {
    Log::Die("TrackingMessenger sent unknown set command: " +
             command->GetCommandPath());
  }
}
