...
size_t HitData::size() {
    return 1*sizeof(uint32_t)*numphotons;
}

Chroma::Chroma(std::string conn) : context(1), socket(context, ZMQ_REQ) {
    socket.connect(conn.c_str());
}
...
void Chroma::addPhoton(const G4ThreeVector &pos, const G4ThreeVector &dir, const G4ThreeVector &pol, const double energy, const double t, const uint32_t trackid) {
    photons.numphotons++;
