...
public:
  ...
  // Sampling rate
  virtual void SetSamplingRate(Float_t _sampling_rate) {
  ...
  // Dynamic range (V)
  virtual void SetDynamicRange(float _dynamic_range) {
    dynamic_range = _dynamic_range;
  };
  virtual Float_t GetDynamicRange() const { return dynamic_range; };

  /// Set a waveform, overwrites existing
  virtual void SetWaveform(const UShort_t waveformID,
                           const std::vector<UShort_t> &samples) {
    waveforms[waveformID] = samples;
  }

  // Get a map of waveform IDs to digitized waveforms
  virtual std::map<UShort_t, std::vector<UShort_t>> GetAllWaveforms() const {
    return waveforms;
  }

  // Get the waveform for a digitizer
  virtual std::vector<UShort_t> GetWaveform(const UShort_t waveformID) const {
    return waveforms.at(waveformID);
  }

  /// Check if a waveform exists
  Bool_t ExistsWaveform(const UShort_t waveformID) const {
    return waveforms.count(waveformID) > 0;
  }
...
protected:
  ...
  std::map<UShort_t, std::vector<UShort_t>>
      waveforms; ///< Map of input number to samples
};
