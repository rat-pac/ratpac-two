...
/// virtually re-written since then. The ES cross-section is now determined in a
...
///   - Updated to accommodate the new solar generator, which is now used by
///   default.
///   - Completely refactored to improve the cross-section calculation on a
///   different class (imported from QSNO).
///   - Prepare ground to return both the electron in the neutrino in the
///   generated event (only electron for now).
/// - 02-Jul-2012 N. Barros:
///   - Removed the usage of Geant4 default streamers (G4cout) to use RAT Log
///   mechanism instead.
/// - 02-Aug-2012 N. Barros:
///   - Reintroduced the compatibility with the combo generator, for simulation
///   of simple cases.
///   - The messenger options are available or not depending on who is the
///   parent generator (disabled if it is solar).
...
///   - If the direction is set to 0,0,0 the generator now produces a random
///   direction at each event.
...
class VertexGen_ES : public GLG4VertexGen {
 public:
   ...
   *
   * @param flavor Flavor key passed to the RAT::ESCrossSec class. It can be one
