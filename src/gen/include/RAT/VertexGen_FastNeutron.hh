 ...
 *
 * This vertex generator was based on the template from Jeanne Wilson and
 * adapted for external fast- neutrons. One of the assumption made is that the
 * flux emmited from the wall follow the Mei and Hime formulation. A zeroth
 * order assumption is that the neutron angle from the muon track are taken from
 * the average muon direction which is (0,0,-1). This assumption will need to be
...
class VertexGen_FastNeutron : public GLG4VertexGen {
 public:
  ...
  /** Set up the FastNeutron array in memory - create normalised cumulative
   * magnitude for easy**/
