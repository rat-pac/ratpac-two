...
class GLG4GenList : public std::priority_queue<GLG4Gen *, std::vector<GLG4Gen *>, TimeOrder_GLG4Gen> {
 public:
  inline void SubtractTime(double time) {
    for (std::vector<GLG4Gen *>::iterator i = c.begin(); i != c.end(); i++) (*i)->SubtractTime(time);
  };
