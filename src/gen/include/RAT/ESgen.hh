...
/// \date 15-Dec-2010
///
/// \brief Implements the generation of a neutrino-electron elastic scattering
/// event.
///
...
/// implementation by Joe Formaggio and the following adaptation by Bill
/// Seligman. However the whole class was re-written later to adapt it for the
/// final use of solar neutrino generation.
///
/// REVISION HISTORY:\n
...
///         event, while this class deals with the physics of the cross-section.
///         Some of the code (the flux in particular) is copied from IBDgen.
///     - 15-Dec-2010 N. Barros (LIP)
///         - Strongly re-wrote this class to accommodate a solar neutrino
///         generator. (solar). The original implementation was generating ES
///         events from reactor anti-nus (ibd).
///         - The code has been strongly refactored so that the original code is
///         still running. The choice is passed through the macro file or the
///         command line.
///     - 18-Feb-2011 N. Barros:
///         - Improved the code for speed.
///         - Solved a minor bug in the calculation of the direction of the
///         outgoing electron.
///         - Moved Geant4-related calculations into the parent vertex generator
///         RAT::VertexGen_ES.
///     - 14-Dec-2011 N. Barros:
///         - Improved usage of the incoming neutrino direction and
///         corresponding calculation of outgoing electron.
///     - 10-Jan-2012 N. Barros:
///         - Solved a problem with the rotation applied to get the outgoing
///         electron direction.
...
///         - Improved speed on random sampling over the spectrum and cross
///         sections.
///     - 02-Jul-2012 N. Barros:
///         - Removed now superfluous SetXSecMax(). Now using CLHEP random
///         sampler.
///         - Solved problem in the determination of the rater per target for
///         continuous spectra.
///         - Removed dependency on ESMessenger. Its functionality was moved to
///         upper level classes.
///     - 14-Aug-2012 N. Barros:
///         - Solved some issues with the compatibility with the combo
///         generator.
///         - If the direction is set to 0,0,0 the generator now produces a
///         random direction at each event.
///         - Added a new flag to customize the name of the database to draw the
///         spectrum from (allows ad-hoc entries).
