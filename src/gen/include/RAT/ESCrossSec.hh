...
///
/// 22-JUN-2012 - Nuno Barros   - Cleaned up code and solved a small problem
/// with unit conversion.
///
/// 07-JUL-2012 - Nuno Barros   - Removed all Geant4 streamers replacing them by
/// RAT log objects.
///                             - Changed geant4 data types by C++ data types
///                             (suggestion by CIC).
///                             - Revised constness of members.
///
///
/// \details Calculates the neutrino-electron ES cross section.
///         The calculation is performed either for the ES_e (W+Z) channel, or
///         the ES_mu,tau (Z) channel. Some remarks concerning the calculations.
///         \param E,Enu    : Neutrino energy (MeV).
///         \param T,Te     : Recoil electron energy (MeV).
///         \return Cross section in units of $10^{-42}cm^{2}$
///
///         Available strategies for the ES cross section calculation (set by
///         messenger):
///         - 1 : Original routine from QSNO::PNuE (Bahcall).
///         - 2 : Improved routine from QSNO::PNuE (without rad. corrections).
///         - 3 : Improved routine from QSNO::PNuE (with rad. corrections -
///         analytical).
///         - 4 (default) : Improved routine from QSNO::PNuE (with rad.
///         corrections - table).
///
///
////////////////////////////////////////////////////////////////////
...
class ESCrossSec {
 public:
  ...
  /**
   * @brief Calculate the differential cross section for the neutrino energy
   * Enu.
   *
   * @param Enu Incoming neutrino energy (MeV).
   * @param Te  Recoil electron energy (MeV).
   * @return Differencial cross section \f$ \frac{d\sigma}{dT} \f$ in units of
   * \f$ 10^{-42} cm^{2} \f$ .
   */
   ...
   * @return  \f$ \frac{d\sigma}{d\cos \theta} \f$ in units of \f$ 10^{-42}
   * cm^{2} \f$ .
   */
  ...
  /**
   * @brief Return a TGraph with the differential cross section for an incoming
   * neutrino with energy Enu.
   * @param Enu Incoming neutrino energy (MeV)
   * @return TGraph with the shape of \f$ \frac{d\sigma}{dT} \f$ in units of \f$
   * 10^{-42} cm^{2} \f$ .
   */
