 ...
 *
 * Top Level Event Producer - coincidences of different interactions separated
 * in time and space in same event
 *
 * @author Jeanne Wilson <j.wilson2@physics.ox.ac.uk>
 *
 * first version : 16-July-2008
 *
 * This generator simulates 2 different decays at different positions and
 * different times in the same specified event window. It can be used for the
 * purposes of studying pile-up as it will efficiently produce a sample in which
 * each event is a coincidence. There is also the option of restricting the
 * events generated to those with summed primary particle energy in a given
 * range.
 */
...
class Coincidence_Gen : public GLG4Gen {
 public:
   ...
   * this option is called the user must set a time constant, T, for each added
   * vertex. First vertex is at t = 0, the rest have times selected from
   * exponentials exp(-t/T) each exponential applies to the time since last
   * vertex */
 ...
 protected:
  ...
  double fTimeWindow;
  /** choose exponential timing structure (either all vertices have random
   * times, or all are given decay constants*/
  bool _exptiming;
  bool _expTimingTimeWindow;
  bool _expTimingTimeWindowForce;
  /** the value of an exponential decay factor for subsequent decays - default
   * is zero, only used if set */
  double fExponent[5];
