...
void Gen_LED::GenerateEvent(G4Event *event) {
  ...
  G4ThreeVector pos(led_x[next_led] * CLHEP::mm, led_y[next_led] * CLHEP::mm,
                    led_z[next_led] * CLHEP::mm);
  ...
  if (!oneLED) { // fire all LEDs in same event
    for (int iLED = 0; iLED < int(led_x.size()); iLED++) {
      pos.set(led_x[iLED] * CLHEP::mm, led_y[iLED] * CLHEP::mm,
              led_z[iLED] * CLHEP::mm);
      normal = -pos.unit();
      ...
      for (int i = 0; i < photons_per_LED[iLED]; i++) {
        ...
        if (iso_mode)
          ...
          theta = rand_angles[iLED]->shoot() *
                      (angle_maxs[iLED] - angle_mins[iLED]) +
                  angle_mins[iLED];
        else {
          warn << "Warning: missing " << next_led << "-th angular distr., only "
               << rand_angles.size() << " exist! Reusing last distrib.\n";
          theta = rand_angles[rand_angles.size() - 1]->shoot() *
                      (angle_maxs[rand_angles.size() - 1] -
        ...
        G4PrimaryParticle *particle =
            new G4PrimaryParticle(photonDef, mom.x(), mom.y(), mom.z());
        // set polarization
    ...
    for (int i = 0; i < photons_per_event; i++) {
      ...
      if (iso_mode)
        ...
        theta = rand_angles[next_led]->shoot() *
                    (angle_maxs[next_led] - angle_mins[next_led]) +
                angle_mins[next_led];
      else {
        warn << "Warning: missing " << next_led << "-th angular distr., only "
             << rand_angles.size() << " exist! Reusing last distrib.\n";
...
void Gen_LED::SetState(G4String state) {
  ...
  if (DB::Get()->GetDefaultTable("LED", state) ||
      DB::Get()->GetUserTable("LED", state))
    stateStr = state;
  else
    stateStr = "default";
...
void Gen_LED::SetLEDParameters(G4String state) {
  ...
  Log::Assert((led_x.size() == led_y.size() && led_y.size() == led_z.size()),
              "Some LEDs miss some coordinate(s)\n");
  Log::Assert((led_x.size() <= led_wavelength.size()),
              "Some LEDs miss a wavelength\n");
  ...
  if (intensity_mode == "single")
    ...
    oneLED = false;
  if (oneLED == false)
    Log::Assert((led_x.size() <= photons_per_LED.size()),
                "Some LEDs miss their intensity\n");
  ...
  if (wl_mode == "mono")
    ...
    Log::Die(
        dformat("Gen_LED: LED.wl_mode = \"%s\" is invalid.", wl_mode.c_str()));
  ...
  if (!iso_mode) {
    ...
    if (!multi_ang_dist_mode) {
      ...
      if (int(led_x.size()) != lled->GetI("n_ang_dists"))
        warn << lled->GetI("n_ang_dists") << " angular distributions, but "
             << led_x.size() << " LEDs...!\n";
      double step = -.1;
      angle_maxs.clear();
      angle_mins.clear();
      ...
      if (int(rand_angles.size()) != lled->GetI("n_ang_dists"))
        warn << "Loaded " << rand_angles.size()
             << " angular distributions. Should be "
             << lled->GetI("n_ang_dists")
             << "... Data file needs to be rechecked!\n";
      for (int iangs = 0; iangs < int(rand_angles.size()); iangs++) {
        Log::Assert(
            rand_angles[iangs] != NULL,
            dformat("Failed loading an angular distribution for %d-th LED",
                    iangs));
      }
    }
  }
...
void Gen_LED::SetTimeState(G4String state) {
  if (timeGen)
    ...
    G4cerr << "Gen_LED error: Cannot set time state, no time generator selected"
           << G4endl;
}
...
void Gen_LED::SetPosState(G4String /*state*/) {
  G4cerr << "Gen_LED error: Cannot set position state, no position generator"
         << G4endl;
}
