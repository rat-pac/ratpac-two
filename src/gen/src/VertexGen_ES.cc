...
void VertexGen_ES::GeneratePrimaryVertex(G4Event *argEvent, G4ThreeVector &dx, G4double dt) {
  //
...
void VertexGen_ES::SetState(G4String newValues) {
  ...
  switch (params.size()) {
    ...
    case 1:
      this->SetDBName(params[0]);
    default:
      info << "VertexGen_ES : Detected only " << params.size() << " neutrino state terms (1,2, or 3 expected)."
           << newline;
      return;
  }
}
