...
VertexGen_ES::VertexGen_ES(const char *arg_dbname)
    : GLG4VertexGen(arg_dbname), fNuDir(0., 0., 0.), fDBName("SOLAR"),
      fRandomDir(false) {
...
void VertexGen_ES::GeneratePrimaryVertex(G4Event *argEvent, G4ThreeVector &dx,
                                         G4double dt) {
  //
  ...
  electron_particle->SetMass(
      fElectronMass); // This seems to help in VertexGen_IBD
  ...
  if (this->GetNuFlavor() == "nue") {
    neutrinoparent =
        new G4PrimaryParticle(fNue, mom_nu.px(), mom_nu.py(), mom_nu.pz());
  } else if (this->GetNuFlavor() == "numu") {
    neutrinoparent =
        new G4PrimaryParticle(fNumu, mom_nu.px(), mom_nu.py(), mom_nu.pz());
  } else {
    return;
  }
...
void VertexGen_ES::SetState(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    G4cout << "Current state of this VertexGen_ES:\n"
           ...
           << G4endl;
    G4cout
        << "Format of argument to VertexGen_ES::SetState: \n"
  ...
  case 1:
    this->SetDBName(params[0]);
  default:
    G4cout << "VertexGen_ES : Detected only " << params.size()
           << " neutrino state terms (1,2, or 3 expected).\n";
    return;
  }
}
