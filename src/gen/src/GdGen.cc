...
namespace RAT {
    ...
    {
#ifdef DEBUG
        G4cout << "RAT::GdGen::SetState called with state='"
        << state << "'" << G4endl;
#endif
...
#ifdef DEBUG
        G4cout << "RAT::GdGen::SetState: nArgs="
        << nArgs << G4endl;
#endif
        ...
        try {
            ...
            if ( nArgs >= 2 )
            {
                ...
                if ( isotope != 158 )
                {
                    G4cerr << "RAT::GdGen::SetState: Only gd 158 is supported"
                    << G4endl;
                }
            ...
            {
                G4Exception(__FILE__, "Invalid Parameter", FatalException, ("GdGen syntax error: '" +
                                                                            state +
    ...
    void GdGen::SetPosState(G4String state)
    {
        if (posGen)
            ...
            G4cerr << "GdGen error: Cannot set position state, no position generator selected" << G4endl;
    }
