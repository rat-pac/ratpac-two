...
void VertexGen_WIMP::GeneratePrimaryVertex(G4Event *event, G4ThreeVector &dx, G4double dt) {
  // Draw energy from WIMP distribution
  double ke = fEnergyDistLo + (fEnergyDistHi - fEnergyDistLo) * fEnergyDist->shoot();
  ...
  G4PrimaryParticle *nucleus = new G4PrimaryParticle(fNucleus, mom.x(), mom.y(), mom.z());
...
void VertexGen_WIMP::SetState(G4String newValues) {
  ...
  if (is.fail()) Log::Die("VertexGen_WIMP: Incorrect vertex setting " + newValues);
...
G4String VertexGen_WIMP::GetState() { return dformat("%s\t%f", fNucleusName.c_str(), fWIMPMass); }
...
void VertexGen_WIMP::Setup() {
  ...
  const double v_EarthSun =
      lwimp->GetD("v_EarthSun") * CLHEP::km / CLHEP::s;  // Earth's velocity around sun in direction of Sun's velocity
  ...
  if (normal > 0)
    for (int i = 0; i < nsamples; i++) dRdQ[i] /= normal;
