...
void VertexGen_WIMP::GeneratePrimaryVertex(G4Event *event, G4ThreeVector &dx,
                                           G4double dt) {
  // Draw energy from WIMP distribution
  double ke =
      fEnergyDistLo + (fEnergyDistHi - fEnergyDistLo) * fEnergyDist->shoot();
  ...
  G4PrimaryParticle *nucleus =
      new G4PrimaryParticle(fNucleus, mom.x(), mom.y(), mom.z());
...
double VertexGen_WIMP::Helmff(double E, double mA) {
  ...
  return 9 * ((Sin(qr) - qr * Cos(qr)) / pow(qr, 3)) *
         ((Sin(qr) - qr * Cos(qr)) / pow(qr, 3)) * Exp(-qs * qs);
}

// Eq. (19) of C. Savage, G. Gelmini, P. Gondolo, K. Freese, JCAP 0904:010, 2009
double VertexGen_WIMP::VelIntegral(double vmin, double v0, double vE,
                                   double vesc) {
  if (v0 == 0) {
  ...
  if (xesc < xE)
    G4cout << "VertexGen_WIMP warning (we are not escaping the galaxy): our "
              "velocity is "
           << vE << ", the local Milky Way escape velocity is " << vesc
           << ". The results will be unphysical.\n";
  ...
  if (xmin <= Abs(xesc - xE)) {
    ...
    if (xesc <= xE)
      return 1 / vE;
    if (xesc > xE)
      return (0.25 * sqrt(CLHEP::pi) / xE * (erf(xmin + xE) - erf(xmin - xE)) -
              exp(-xesc * xesc)) /
             (0.5 * sqrt(CLHEP::pi) * erf(xesc) - xesc * exp(-xesc * xesc)) /
             v0;
  }
  // this in case the tail is the best we can get
  return (0.25 * sqrt(CLHEP::pi) * (erf(xesc) - erf(xmin - xE)) -
          .5 * (xesc + xE - xmin) * exp(-xesc * xesc)) /
         xE / (0.5 * sqrt(CLHEP::pi) * erf(xesc) - xesc * exp(-xesc * xesc)) /
         v0;
}
...
void VertexGen_WIMP::Setup() {
  ...
  // Useful constants
  const double v_esc = lwimp->GetD("v_esc") * CLHEP::km /
                       CLHEP::s; // Local galactic escape velocity
  const double v_0 =
      lwimp->GetD("v_0") * CLHEP::km / CLHEP::s; // Mean galactic WIMP velocity
  const double v_SunGal = lwimp->GetD("v_SunGal") * CLHEP::km /
                          CLHEP::s; // Sun's velocity around galactic center
  const double v_EarthSun =
      lwimp->GetD("v_EarthSun") * CLHEP::km /
      CLHEP::s; // Earth's velocity around sun in direction of Sun's velocity
                // around galactic center

  // const double rho_wimp = lwimp->GetD("rho") * GeV / cm3; // Local dark
  // matter halo density

  const double date = lwimp->GetD("date"); // time since January 1 in days
  // Net velocity of Earth through halo, from R.W.Schnee,
  // http://arxiv.org/abs/1101.5205
  const double v_E =
      v_SunGal + v_EarthSun * cos(2 * CLHEP::pi / 365.25 * (date - 152.5));
  ...
  const double r =
      4 * (fWIMPMass * m_nucleus) / pow(fWIMPMass + m_nucleus, 2.0);
  ...
  for (int i = 0; i < nsamples; i++) {
    ...
    double rate = Helmff(ke, m_nucleus) * R_0 / (r * E_0) *
                  VelIntegral(v_min, v_0, v_E, v_esc);
