...
namespace RAT {
  ...
  {
#ifdef DEBUG
    G4cout << "RAT::AmBeGen::SetState called with state='"
	   << state << "'" << G4endl;
#endif
...
#ifdef DEBUG
    G4cout << "RAT::AmBeGen::SetState: nArgs="
	   << nArgs << G4endl;
#endif
	...
	{
	  G4Exception(__FILE__, "Invalid Parameter", FatalException, ("AmBeGen syntax error: '" +
								      state +
  ...
  G4String AmBeGen::GetState() const
  {
    return stateStr;
  }
  ...
  void AmBeGen::SetPosState(G4String state)
  {
    if (posGen)
      ...
      G4cerr << "AmBeGen error: Cannot set position state, no position generator selected" << G4endl;
  }
