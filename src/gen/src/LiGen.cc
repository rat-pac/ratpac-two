...
void LiGen::GenerateEvent(G4Event* event)
{
    ...
    for(int i=0;i<numElectron;i++){
        ...
        for(int j=0;j<sizeof(brArrLi)/sizeof(brArrLi[0]);j++){
            if(randNow<cdfLi[j]){idxNow=j;break;}
        }
...
{
#ifdef DEBUG
    G4cout << "RAT::LiGen::SetState called with state='"
    << state << "'" << G4endl;
#endif
...
#ifdef DEBUG
    G4cout << "RAT::LiGen::SetState: nArgs="
    << nArgs << G4endl;
#endif
    ...
    try {
        ...
        if ( nArgs >= 2 )
        {
            ...
            if ( isotope != 9 )
            {
                G4cerr << "RAT::LiGen::SetState: Only Li 9 is supported"
                << G4endl;
            }
        ...
        {
            G4Exception(__FILE__, "Invalid Parameter", FatalException, ("LiGen syntax error: '" +
                                                                        state +
...
void LiGen::SetTimeState(G4String state)
{
    if (timeGen)
        ...
        G4cerr << "LiGen error: Cannot set time state, no time generator selected" << G4endl;
}
...
void LiGen::SetPosState(G4String state)
{
    if (posGen)
        ...
        G4cerr << "LiGen error: Cannot set position state, no position generator selected" << G4endl;
}
