...
LiGen::LiGen() : stateStr(""), isotope(9), posGen(0) {
  ...
  pdfNow = 0.;
  sumBr = accumulate(std::begin(brArrLi), std::end(brArrLi), 0.,
                     std::plus<double>());
...
void LiGen::SetState(G4String state) {
#ifdef DEBUG
  G4cout << "RAT::LiGen::SetState called with state='" << state << "'"
         << G4endl;
#endif
...
#ifdef DEBUG
  G4cout << "RAT::LiGen::SetState: nArgs=" << nArgs << G4endl;
#endif
  ...
  try {
    ...
    if (nArgs >= 2) {
      ...
      if (isotope != 9) {
        G4cerr << "RAT::LiGen::SetState: Only Li 9 is supported" << G4endl;
      }
...
void LiGen::SetTimeState(G4String state) {
  if (timeGen)
    ...
    G4cerr << "LiGen error: Cannot set time state, no time generator selected"
           << G4endl;
}
...
void LiGen::SetPosState(G4String state) {
  if (posGen)
    ...
    G4cerr << "LiGen error: Cannot set position state, no position generator "
              "selected"
           << G4endl;
}
