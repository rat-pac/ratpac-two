...
VertexGen_IBD::VertexGen_IBD(const char *arg_dbname) : GLG4VertexGen(arg_dbname), nu_dir(0., 0., 0.) {
...
void VertexGen_IBD::GeneratePrimaryVertex(G4Event *argEvent, G4ThreeVector &dx, G4double dt) {
...
void VertexGen_IBD::SetState(G4String newValues) {
  newValues = util_strip_default(newValues);  // from GLG4StringUtil
  if (newValues.length() == 0) {
    // print help and current state
    info << "Current state of this VertexGen_IBD:" << newline << " \"" << GetState() << "\"" << newline << newline;
    info << "Format of argument to VertexGen_IBD::SetState: " << newline << " \"nu_dir_x nu_dir_y nu_dir_z\"\n"
...
G4String VertexGen_IBD::GetState() {
