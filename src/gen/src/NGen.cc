...
void NGen::GenerateEvent(G4Event* event)
{
    ...
    for(int i=0;i<numElectron;i++){
        ...
        for(int j=0;j<sizeof(brArrN)/sizeof(brArrN[0]);j++){
            if(randNow<cdfN[j]){idxNow=j;break;}
        }
...
{
#ifdef DEBUG
    G4cout << "RAT::NGen::SetState called with state='"
    << state << "'" << G4endl;
#endif
...
#ifdef DEBUG
    G4cout << "RAT::NGen::SetState: nArgs="
    << nArgs << G4endl;
#endif
    ...
    try {
        ...
        {
            G4Exception(__FILE__, "Invalid Parameter", FatalException, ("NGen syntax error: '" +
                                                                        state +
...
void NGen::SetPosState(G4String state)
{
    if (posGen)
        ...
        G4cerr << "NGen error: Cannot set position state, no position generator selected" << G4endl;
}
