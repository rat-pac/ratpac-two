...
void Decay0::GenBBTest() {
  ...
  try {
    fEbindeL = fLdecay->GetD("EbindeL");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fEbindeM = fLdecay->GetD("EbindeM");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fEnGamma = fLdecay->GetDArray("EnGamma");
  } catch (DBNotFoundError &e) {
    ;
  }
  fEK = fLdecay->GetD("EK");
...
void Decay0::GenBackgTest() {
  ...
  try {
    fHLifeTime = fLdecay->GetDArray("Thnuc");
  } catch (DBNotFoundError &e) {
  ...
  try {
    fEbindeK2 = fLdecay->GetD("EbindeK2");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
  ...
  try {
    fEbindeL2 = fLdecay->GetD("EbindeL2");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
  ...
  try {
    fEbindeM2 = fLdecay->GetD("EbindeM2");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fDaughterZ = fLdecay->GetIArray("Zdtr");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fShCorrFactor = fLdecay->GetDArray("SCorF");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fProbDecay = fLdecay->GetDArray("ProbDecay");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fProbAlpha = fLdecay->GetDArray("ProbAlpha");
  } catch (DBNotFoundError &e) {
    ;
  }
  ...
  try {
    fEnAlpha = fLdecay->GetDArray("EnAlpha");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fProbEC = fLdecay->GetDArray("ProbEC");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
  ...
  try {
    fProbBeta = fLdecay->GetDArray("ProbBeta");
  } catch (DBNotFoundError &e) {
    ;
  }
  try {
    fEndPoint = fLdecay->GetDArray("EndPoint");
  } catch (DBNotFoundError &e) {
    ;
  }
}
...
void Decay0::GenEvent() {
  ...
  if (fIsotope == "Ac228")
    ...
    fPtime[fNbPart0 + 1] = fPtime[fNbPart0 + 1] + fTdnuc;
    // set the time of the following 208Tl-decay emitted particles to be after
    // the first one
    int i = 0;
    ...
    // information from
    // http://www.nndc.bnl.gov/nudat2/decaysearchdirect.jsp?nuc=10C&unc=nds K
    // binding energy from: https://userweb.jlab.org/~gwyn/ebindene.html
    ...
    // information from
    // http://www.nndc.bnl.gov/nudat2/decaysearchdirect.jsp?nuc=11C&unc=nds
    beta(fEndPoint[0], 0., 0.);
    ...
    Eu147();
  else if (fIsotope == "Eu152")
    Eu152();
  else if (fIsotope == "Eu154")
    Eu154();
  else if (fIsotope == "Gd146")
    Gd146();
  else if (fIsotope == "He6") {
    // information from
    // http://www.nndc.bnl.gov/nudat2/decaysearchdirect.jsp?nuc=6HE&unc=nds
    beta(fEndPoint[0], 0., 0.);
  ...
  } else if (fIsotope == "I126")
    I126();
  else if (fIsotope == "I133")
    I133();
  else if (fIsotope == "I134")
    I134();
  else if (fIsotope == "I135")
    I135();
  else if (fIsotope == "K40")
    K40();
  else if (fIsotope == "K42")
    K42();
    ...
    Sb125();
  else if (fIsotope == "Sb126")
    Sb126();
  else if (fIsotope == "Sb133")
    Sb133();
    ...
    Te133();
  else if (fIsotope == "Te133m")
    Te133m();
  else if (fIsotope == "Te134")
    Te134();
  else if (fIsotope == "Th234")
    Th234();
    ...
    // (E_exc=44 keV) with T1/2=2.4e20 y in accordance with: H.Heiskanen et
    // al.,JPG 34(2007)837. Transition to the ground state (6+) is suppressed by
    // factor of ~1e-9, to the excited 4+ (E_exc=146 keV) - by 1e-2. VIT,
    // 07.05.1998. VIT, 18.08.2007:update to NDS 68(1993)165 and ENSDF at NNDC
    // site.
    fZdtr = fDaughterZ[0];
    beta(fEndPoint[0], 0., 0.);
...
void Decay0::GenBBDeex() {
  ...
  fTclev = 0.;
  if (fIsotope == "Ca48" && fLevelE[fLevel] != 0.)
    Ti48low();
  else if (fIsotope == "Ni58" && fLevelE[fLevel] != 0.)
    Fe58low();
  else if (fIsotope == "Ge76" && fLevelE[fLevel] != 0.)
    Se76low();
  else if (fIsotope == "Se74" && fLevelE[fLevel] != 0.)
    Ge74low();
  else if (fIsotope == "Se82" && fLevelE[fLevel] != 0.)
    Kr82low();
  ...
  } else if (fIsotope == "Zr96" && fLevelE[fLevel] != 0.)
    Mo96low();
  else if (fIsotope == "Mo92" && fLevelE[fLevel] != 0.)
    Zr92low();
  else if (fIsotope == "Mo100" && fLevelE[fLevel] != 0.)
    Ru100low();
  else if (fIsotope == "Ru96" && fLevelE[fLevel] != 0.)
    Mo96low();
  else if (fIsotope == "Ru104" && fLevelE[fLevel] != 0.) {
    fThlev = 9.9e-12;
    nucltransK(fEnGamma[0], fEbindeK, 4.5e-3, 0.);
  } else if (fIsotope == "Cd106" && fLevelE[fLevel] != 0.)
    Pd106low();
  else if (fIsotope == "Cd116" && fLevelE[fLevel] != 0.)
    Sn116low();
  else if (fIsotope == "Sn112" && fLevelE[fLevel] != 0.)
    Cd112low();
  else if (fIsotope == "Sn124" && fLevelE[fLevel] != 0.)
    Te124low();
    ...
    Sm148low();
  else if (fIsotope == "Nd150" && fLevelE[fLevel] != 0.)
    Sm150low();
...
void Decay0::bb() {
  ...
  do {
    if (fModebb == 10)
      fE1 = fEbb1 + (fEbb2 - fEbb1) * GetRandom();
    else if (fModebb != 10)
      fE1 = fEbb2 * GetRandom();
...
void Decay0::Ti48low() {
  ...
  if (fLevel == 2) {  // 2421.
    ...
    p = 100. * GetRandom();
    if (p < 8)
      nucltransK(fEnGamma[0], fEbindeK, 1.5e-5, 5.0e-4);
    else {
...
void Decay0::Te124low() {
  // Subroutine describes the deexcitation process in Te124 nucleus after
...
void Decay0::Ac228() {
  ...
  if (pbeta <= fProbBeta[0]) {  // 0.06%
    ...
    // this transition. In the NDS, 228Ac directly populates the 1531 keV level
    // (producing a 592 keV beta) and not the 1588 keV level, which is instead
    // populated by 228Pa. In order to include the observed gammas below, A.
    // Pearce (author of the nucleide.org reference), replaced the 1588 keV
    // level for the 1531 keV. (See note under Decay Scheme here:
...
void Decay0::Be11() {
  // information from Phys. Rev. C, Vol. 26, Number 3 and JEFF 3.1.1 entry for
  ...
  if (next7286 == true) {
    nucltransK(fEnGamma[4], fEbindeK, 0.0, 0.0);
  }
  if (next2895 == true) {
    nucltransK(fEnGamma[9], fEbindeK, 0.0, 0.0);
  }
...
void Decay0::beta2f(double Qbeta, double tcnuc, double thnuc, int kf, double c1, double c2, double c3, double c4) {
  ...
  // k=4:
  // cf(e)=pel**8+c1*pel**6*pnu**2+c2*pel**4*pnu**4+c3*pel**2*pnu**6+c4*pnu**8,
