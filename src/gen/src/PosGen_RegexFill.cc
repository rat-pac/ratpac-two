...
void PosGen_RegexFill::SetState(G4String regex) {
  ...
  if (regcomp(&re, regex, REG_EXTENDED | REG_NOSUB) != 0) {
    Log::Die("PosGen_RegexFill::SetState: the string \"" + regex +
  ...
  if (fVolumes.size() == fVolumeCumu.size()) {
    Log::Die("PosGen_RegexFill::SetState: the regex \"" + regex +
             "\" did not match any volumes!");
  }
...
void PosGen_RegexFill::FindVolumes(G4LogicalVolume *mother, regex_t *re,
                                   vector<FillVolume> &found) {
  for (int i = 0; i < mother->GetNoDaughters(); i++) {
    ...
    G4AffineTransform transform(daughterPhys->GetRotation(),
                                daughterPhys->GetTranslation());
    if (regexec(re, daughterPhys->GetName().c_str(), (size_t)0, NULL, 0) ==
        0) { // daughterPhys matches the regular expression
      ...
      for (int j = 0; j < daughterLog->GetNoDaughters(); j++) {
        ...
        G4AffineTransform grandTransform(grandPhys->GetRotation(),
                                         grandPhys->GetTranslation());
        ...
        vol.daughters.push_back(
            make_pair(grandSolid, grandTransform.Inverse()));
      }
      ...
      vol.boundVolume =
          (vol.x1 - vol.x0) * (vol.y1 - vol.y0) * (vol.z1 - vol.z0);
    }
    size_t j = found.size();
    FindVolumes(daughterLog, re, found);
    for (; j < found.size(); j++) { // apply this mother->daughterPhys transform
                                    // to all found matches
...
G4String PosGen_RegexFill::GetState() const { return fState; }
...
void PosGen_RegexFill::GeneratePosition(G4ThreeVector &pos) {
  ...
  for (; volidx < fVolumes.size(); volidx++) {
    if (randVolume <= fVolumeCumu[volidx])
      break;
  }
  ...
  for (;;) {
    // Solid works in "local" coordinates, so generate position, test, then
    // project to global
    G4ThreeVector trial(vol.x0 + G4UniformRand() * (vol.x1 - vol.x0),
    ...
    if (solid->Inside(trial)) { // inside the volume but maybe in a daughter
      ...
      for (size_t j = 0; j < vol.daughters.size(); j++) {
        if (vol.daughters[j].first->Inside(
                vol.daughters[j].second.TransformPoint(trial))) {
