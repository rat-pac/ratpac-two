...
G4String GLG4PosGen_Point::GetState() const {
  return util_dformat("%ld\t%ld\t%ld", _fixedPos.x(), _fixedPos.y(),
                      _fixedPos.z());
}
...
GLG4PosGen_Paint::GLG4PosGen_Paint(const char *arg_dbname)
    : GLG4PosGen(arg_dbname), _pos(0., 0., 0.), _thickness(0),
...
void GLG4PosGen_Paint::GeneratePosition(G4ThreeVector &argResult) {
  ...
  if (pv != _pVolume) {  // normally happens just once after a SetState()
    if (_pVolume == 0) { // SetState() sets _pVolume==0
      ...
      if (_pVolumeName != _pVolume->GetName()) {
        G4cerr << "Warning: actual volume at " << _pos << " is "
               << _pVolume->GetName() << ", not equal to expected volume "
               << _pVolumeName << " in GLG4PosGen_Paint." << G4endl;
      }
    } else {
      G4cerr << "Warning: actual volume at " << _pos << " has changed! Now "
  ...
  for (int iloop = 0, jloop = 0; /* test inside */; iloop++) {
    ...
    if (iint >= _intercepts.size()) {
      double Rsphere =
          0.50001 * sqrt(dx * dx + dy * dy + dz * dz) + surfaceTolerance;
      ...
      while (iint >= _intercepts.size()) {
        ...
        for (int isafety = 0; isafety < 20; isafety++) {
          ...
          if (dist >= 2.0 * Rsphere)
            break;
          if (dist <= 0.0) {
            G4cerr << "GLG4PosGen_PointPaintFill: strange DistanceToIn " << dist
...
void GLG4PosGen_Paint::SetState(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    G4cout << "Current state of this GLG4PosGen_Paint:\n"
           ...
           << G4endl;
    G4cout
        << "Format of argument to GLG4PosGen_Paint::SetState: \n"
...
G4String GLG4PosGen_Paint::GetState() const {
  ...
  os << _pos.x() << ' ' << _pos.y() << ' ' << _pos.z() << ' ' << volname << ' '
     << _thickness << ' ' << _materialName << G4std::ends;
  G4String rv(os.str());
  return rv;
}
...
GLG4PosGen_Fill::GLG4PosGen_Fill(const char *arg_dbname)
    : GLG4PosGen(arg_dbname), _pos(0., 0., 0.), _pVolumeName("!"), _pVolume(0),
      _ntried(0), _nfound(0), _boundingBoxVolume(0.0) {}
  ...
  if (pv != _pVolume) {  // normally happens just once after a SetState()
    if (_pVolume == 0) { // SetState() sets _pVolume==0
      ...
      if (_pVolumeName != _pVolume->GetName()) {
        G4cerr << "Warning: actual volume at " << _pos << " is "
               << _pVolume->GetName() << ", not equal to expected volume "
               << _pVolumeName << " in GLG4PosGen_Fill." << G4endl;
      }
    } else {
      G4cerr << "Warning: actual volume at " << _pos << " has changed! Now "
             << pv->GetName() << ", was " << _pVolume->GetName()
...
void GLG4PosGen_Fill::SetState(G4String newValues) {
  ...
  if (newValues == "volume?") {
    if (_ntried <= 0) {
      ...
      G4cout << "  bounding box volume: " << _boundingBoxVolume / CLHEP::meter3
             << " m^3\n";
  ...
  if (!_volumeInfoLoaded) {
    G4Navigator *theNavigator =
        G4TransportationManager::GetTransportationManager()
            ->GetNavigatorForTracking();
    G4VPhysicalVolume *thePhyVolume =
        theNavigator->LocateGlobalPointAndSetup(argResult);
