...
G4String VertexFile_Gen::GetState() const { return fStateStr; }

void VertexFile_Gen::SetTimeState(G4String state) {
  if (fTimeGen)
    fTimeGen->SetState(state);
  else
    warn << "VertexFile_Gen error: Cannot set time state, no time generator "
            "selected\n";
}
...
void VertexFile_Gen::SetPosState(G4String state) {
  if (fPosGen)
    ...
    warn << "VertexFile_Gen error: Cannot set position state, no position "
            "generator selected\n";
}
