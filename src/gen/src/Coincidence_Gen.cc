...
Coincidence_Gen::Coincidence_Gen() : stateStr(""), vertexGen(0), posGen(0) {
  ...
  _expTimingTimeWindow =
      false; // by default, the time window for exponential is not defined.
  _expTimingTimeWindowForce =
      false; // by default, the time window for exponential is not defined.
...
void Coincidence_Gen::GenerateEvent(G4Event *event) {
  ...
  if (_exptiming) {
    ...
    for (int iEx = 0, counter = 0; iEx < nExtra; ++iEx) {
      lastt += CLHEP::RandExponential::shoot(
          fExponent[iEx]); // Each inter-arrival time has it own exponential
                           // time constant
      // if the events are forced in the window, we restart the loop as soon as
      // they are exceeding the window
      if (_expTimingTimeWindowForce && lastt > fTimeWindow) {
        ...
        if (counter < 100) {
          ...
          info << "WARNING: ---Coincidence Generator: too many trials, keeping "
                  "the last one and not simulating the events outside the "
                  "window---"
               << newline;
          break;
        }
  ...
  if (fLoEnergy > 0 || fHiEnergy < 99999) {
    ...
    if (lo < 0)
      lo = 0;
    if (hi < 0) {
      G4cerr << "Coincidence_Gen: cannot achieve selected energy range with "
                "these generators."
             << G4endl;
    }
    if (vertexGen->ELimitable()) {
      ...
      // we will have to apply trial and error so generate temporary event and
      // test it
      G4double testE = -999 * CLHEP::MeV;
      int count = 1;
      while (count < 100 &&
             !(testE / CLHEP::MeV > lo && testE / CLHEP::MeV < hi)) {
        ...
        for (int iP = 0; iP < vtemp->GetNumberOfParticle(); ++iP) {
          ...
          testE += sqrt(PP->GetMomentum().mag2() + pow(PP->GetMass(), 2)) -
                   PP->GetMass();
    ...
    for (int iP = 0; iP < vtemp->GetNumberOfParticle(); ++iP) {
      PP = vtemp->GetPrimary(iP);
      KEgen += sqrt(PP->GetMomentum().mag2() + pow(PP->GetMass(), 2)) -
               PP->GetMass();
    }
  } else {
  ...
  for (int iEx = 0; iEx < nExtra; ++iEx) {
    if (_expTimingTimeWindow && times[iEx + 1] > fTimeWindow)
      continue; // if the time window is defined, reject the events starting
    ...
    if (fLoEnergy > 0 || fHiEnergy < 99999) {
      ...
      if (vertexGenExtra[iEx]->ELimitable()) {
        ...
        while (count < 100 &&
               !(testE / CLHEP::MeV > lo && testE / CLHEP::MeV < hi)) {
          etemp = new G4Event(); // need to reset the temporary event
          vertexGenExtra[iEx]->GeneratePrimaryVertex(etemp, pos,
                                                     times[iEx + 1]);
          vtemp = etemp->GetPrimaryVertex();
          testE = 0;
          for (int iP = 0; iP < vtemp->GetNumberOfParticle(); ++iP) {
  ...
  try {
    ...
    default:
      G4Exception(__FILE__, "Invalid Parameter", FatalException,
                  ("Coincidence generator syntax error: " + state).c_str());
      break;
    }
...
G4String Coincidence_Gen::GetState() const { return stateStr; }
...
void Coincidence_Gen::SetEnergyRange(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    G4cout
        << " Current limits on Coincidence_Gen generated energy range = "
  ...
  if (Elo < Ehi) {
    fLoEnergy = Elo;
    fHiEnergy = Ehi;
    G4cout << "Coincidence_Gen: Restricting generated energy range to " << Elo
           << " - " << Ehi << " MeV\n"
           << " !!!! But note that this option will be very slow for vertex "
              "generators whose energy cannot be limited !!!! "
...
void Coincidence_Gen::SetTimeState(G4String state) {
  if (timeGen) {
    ...
    G4cerr << "Coincidence_Gen error: Cannot set time state, no time generator "
              "selected"
           << G4endl;
  }
}
...
void Coincidence_Gen::SetPosState(G4String state) {
  ...
  if (posGen) {
    ...
    G4cerr << "Coincidence_Gen error: Cannot set position state, no position "
              "generator selected"
           << G4endl;
  }
}
...
void Coincidence_Gen::SetVertexState(G4String state) {
  ...
  if (vertexGen) {
    ...
    G4cerr << "Coincidence_Gen error: Cannot set vertex state, no vertex "
              "generator selected"
           << G4endl;
  }
}
...
void Coincidence_Gen::AddExtra(G4String state) {
  ...
  try {
    ...
    case 2:
      ...
      delete vertexGenExtra[nExtra];
      vertexGenExtra[nExtra] = 0;
...
G4String Coincidence_Gen::GetExtraState(int nint) const {
  if (nint <= nExtra) { // check we have added an extra interaction
    ...
    return G4String(
        "Coincidence_Gen error: that extra interaction has not been selected");
  }
}

/*--------------------------------------------------------------*/
void Coincidence_Gen::SetExtraPosState(G4String state) {
  // set the position generator name for the most recently added extra
  // interaction
  if (nExtra > 0) { // check we have added an extra interaction
    posGenExtra[nExtra - 1]->SetState(state);
    G4cout << "Setting extra interaction " << nExtra << ", position type to "
           << state << G4endl;
  } else {
    G4cerr << "Coincidence_Gen error: Cannot set extra position state, no "
...
void Coincidence_Gen::SetExtraVertexState(G4String state) {
  ...
  if (nExtra > 0) {
    ...
    G4cout << "Setting extra interaction " << nExtra << ", vertex type to "
           << state << G4endl;
  } else {
    G4cerr << "Coincidence_Gen error: Cannot set extra vertex state, no extra "
...
void Coincidence_Gen::SetExponentials(G4String newValues) {
   ...
   * Default is to give each vertex a random time in the event window but if
   * this option is called User must set a time constant for each added vertex.
...
void Coincidence_Gen::SetExpoForceWindow(G4bool newValues) {
  /* Set Force the events to be all in the time window. If set to false, the
   event not inside the window will not be simulated.It is valable for all the
   coincidence interactions, with the time window starting with the first
   event.*/
