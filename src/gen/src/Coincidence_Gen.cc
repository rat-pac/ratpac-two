  ...
  if (_exptiming) {
    ...
    for (int iEx = 0, counter = 0; iEx < nExtra; ++iEx) {
      ...
      if (_expTimingTimeWindowForce && lastt > fTimeWindow) {
        counter++;
        if (counter < 100) {
          times.clear();  // restarting with en empty std::vector
...
void Coincidence_Gen::SetEnergyRange(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    info << " Current limits on Coincidence_Gen generated energy range = " << fLoEnergy << " - " << fHiEnergy << " MeV"
         << newline << " To change use syntax:  Elo  Ehi " << newline
...
void Coincidence_Gen::SetExponentials(G4String newValues) {
  ...
  for (int i = 0; i < nExtra; ++i) {
    is >> fExponent[i];
    if (is.good()) {
      info << "Coincidence_Gen: Setting extra interaction " << i + 1 << " exponential time constant to " << fExponent[i]
           << newline;
    } else {
