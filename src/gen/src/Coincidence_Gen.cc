...
void Coincidence_Gen::GenerateEvent(G4Event *event) {
  ...
  if (_exptiming) {
    ...
    for (int iEx = 0, counter = 0; iEx < nExtra; ++iEx) {
      lastt += CLHEP::RandExponential::shoot(fExponent[iEx]);  // Each inter-arrival time has it own exponential
                                                               // time constant
...
void Coincidence_Gen::SetEnergyRange(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    G4cout << " Current limits on Coincidence_Gen generated energy range = " << fLoEnergy << " - " << fHiEnergy
           << " MeV\n"
...
void Coincidence_Gen::AddExtra(G4String state) {
  ...
  try {
    switch (parts.size()) {
      case 2:
        G4cout << "adding new interaction " << nExtra + 1 << " " << parts[0] << " " << parts[1] << G4endl;
...
void Coincidence_Gen::SetExponentials(G4String newValues) {
  ...
  for (int i = 0; i < nExtra; ++i) {
    is >> fExponent[i];
    if (is.good()) {
      G4cout << "Coincidence_Gen: Setting extra interaction " << i + 1 << " exponential time constant to "
             << fExponent[i] << G4endl;
    } else {
