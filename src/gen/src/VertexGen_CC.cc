...
void VertexGen_CC::GeneratePrimaryVertex(G4Event *argEvent, G4ThreeVector &dx,
                                         G4double dt) {
  //
...
void VertexGen_CC::SetState(G4String newValues) {
  ...
  if (newValues.length() == 0) {
    ...
    G4cout << "Current state of this VertexGen_CC:\n"
           ...
           << G4endl;
    G4cout
        << "Format of argument to VertexGen_CC::SetState: \n"
  ...
  case 1:
    this->SetDBName(params[0]);
  default:
    G4cout << "VertexGen_CC : Detected only " << params.size()
