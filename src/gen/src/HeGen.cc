...
G4double brArrHe[2] = {0.08,  0.08 }; //branching ratio, taking into account the branching to produce neutron
G4double cdfHe[2]   = {0.,0.}; //will be filled later based on branching ratio

namespace RAT {

HeGen::HeGen() :
stateStr(""), isotope(8), posGen(0)
{
...
void HeGen::GenerateEvent(G4Event* event)
{
    ...
    for(int i=0;i<numElectron;i++){
        ...
        for(int j=0;j<sizeof(brArrHe)/sizeof(brArrHe[0]);j++){
            if(randNow<cdfHe[j]){idxNow=j;break;}
        }
    ...
    for(int i=0;i<numGamma;i++){
        ...
        //and when it decays to these excited state, 50% of the time, it emits neutron
...
{
#ifdef DEBUG
    G4cout << "RAT::HeGen::SetState called with state='"
    << state << "'" << G4endl;
#endif
...
#ifdef DEBUG
    G4cout << "RAT::HeGen::SetState: nArgs="
    << nArgs << G4endl;
#endif
    ...
    try {
        ...
        if ( nArgs >= 2 )
        {
            ...
            if ( isotope != 8 )
            {
                G4cerr << "RAT::HeGen::SetState: Only He 8 is supported"
                << G4endl;
            }
        ...
        {
            G4Exception(__FILE__, "Invalid Parameter", FatalException, ("HeGen syntax error: '" +
                                                                        state +
...
void HeGen::SetPosState(G4String state)
{
    if (posGen)
        ...
        G4cerr << "HeGen error: Cannot set position state, no position generator selected" << G4endl;
}
