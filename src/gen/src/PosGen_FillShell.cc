...
PosGen_FillShell::PosGen_FillShell(const char *arg_dbname)
    : GLG4PosGen(arg_dbname), pos(0., 0., 0.), pVolume(0),
      max_iterations(10000000) {}

// In order to get a GEANT4 physical volume by name, we must search through
// the list in the G4PhysicalVolumeStore.
G4VPhysicalVolume *
PosGen_FillShell::FindPhysVolume(const std::string volume_name) {
  G4PhysicalVolumeStore *store = G4PhysicalVolumeStore::GetInstance();
  Log::Assert(store, "PosGen_FillShell::FindPhysVolume: Unable to get GEANT4 "
...
void PosGen_FillShell::SetState(G4String newValues) {
  ...
  case 6:
    ...
    Log::Assert(
        pVolume,
        "PosGen_FillShell::SetState: No physical volume found with name " +
            volname);
    break;
  ...
  case 8:
    ...
    gn = G4TransportationManager::GetTransportationManager()
             ->GetNavigatorForTracking();
    pVolume = gn->LocateGlobalPointAndSetup(vpos, 0, false);
    Log::Assert(
        pVolume,
        "PosGen_FillShell::SetState: No physical volume found at given point");
    break;
...
G4String PosGen_FillShell::GetState() const {
  std::string rv = dformat("%d %d %d %d %d %s", pos.x(), pos.y(), pos.z(), ri,
                           ro, pVolume->GetName().c_str());
  return G4String(rv);
}
...
void PosGen_FillShell::GeneratePosition(G4ThreeVector &argResult) {
  ...
  do {
    ...
    double r =
        pow(G4UniformRand() * (pow(ro, 3.0) - pow(ri, 3.0)) + pow(ri, 3.0),
            1.0 / 3);
    ...
    Log::Assert(iterations < max_iterations,
                ...
                "within volume " +
                    pVolume->GetName() + " in " + ::to_string(max_iterations) +
                    " tries.");

  } while (!(gNavigator->LocateGlobalPointAndSetup(rpos, 0, true)->GetName() ==
             pVolume->GetName()));

  debug << "PosGen_FillShell::GeneratePosition: Point in volume "
        << pVolume->GetName() << " found in " << iterations << " tries.\n";
