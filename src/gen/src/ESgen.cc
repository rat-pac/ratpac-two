...
namespace RAT {
  ...
  ESgen::ESgen() : fNuType(""), fNuFlavor(""),fXS(NULL), fNuSpectrum(NULL), fFluxMax(0.),
  ...
  G4double ESgen::GetRatePerTarget() {
    if (!fGenLoaded) {
      G4Exception("ESgen::GetRatePerTarget","404",FatalErrorInArgument,"Trying to get rate before initializing all parameters.");
    }
