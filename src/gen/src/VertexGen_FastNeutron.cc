...
void VertexGen_FastNeutron::GeneratePrimaryVertex(G4Event *event,
  ...
  G4ThreeVector neutronDirection =
      ev_mu_dir.rotate(acos(cosTheta), rotation_axis);

  // Find the kinetic information of the neutron
  double massN = n->GetPDGMass();
  double totNeutronMomentum =
      sqrt(pow(neutronEnergy + massN, 2) - pow(massN, 2));
...
void VertexGen_FastNeutron::GetMeiHimeParameters(double depth, double emin,
                                                 double &cosTheta,
                                                 double &energy) {
  ...
  TF1 *MeiHimeCosDistribution =
      new TF1("MeiHimeCosDistribution", "[0]/(pow((1-x),[1])+[2])", -1, 1);
...
void VertexGen_FastNeutron::LoadTangHortonSmithCosTheta(double depth) {
  ...
  if (loadedTangHortonSmith)
    return;
  loadedTangHortonSmith = true;
  // Equation (10) of Tang, Horton-Smith PhysRevD.74.053007
  TF1 *cosThetaPrime = new TF1(
      "cosThetaPrime",
      "sqrt((x**2 +[0]**2 + [1]*x**[2] + [3]*x**[4])/(1.0+[0]**2+[1]+[3]))", 0,
      1);
  ...
  TF1 *fL = new TF1(
      "fL", "sqrt(([0]+[1])**2-([0]-[2])**2 * sin(acos(x))**2)- ([0]-[2])*x",
      -1., 1.);

  Double_t density =
      2.65; // Density of rock material used by range equation below
  ...
  // Energy bins
  Double_t energyCSDA[43] = {
      10.0e-3, 14.0e-3, 20.0e-3, 30.0e-3, 40.0e-3, 80.0e-3, 100.e-3, 140.e-3,
      200.e-3, 300.e-3, 400.e-3, 800.e-3, 1.00,    1.40,    2.00,    3.00,
      4.00,    8.00,    10.0,    14.0,    20.0,    30.0,    40.0,    80.0,
      100.,    1,       200.,    300.,    400.,    800.,    1.00e3,  1.40e3,
      2.00e3,  3.00e3,  4.00e3,  8.00e3,  10.0e3,  14.0e3,  20.0e3,  30.0e3,
      40.0e3,  80.0e3,  100.e3};

  // Range in rock of density 2.65
  Double_t rangeCSDA[43] = {
      8.516e-1, 1.542e0, 2.866e0, 5.698e0, 9.145e0, 2.676e1, 3.696e1, 5.879e1,
      9.332e1,  1.524e2, 2.115e2, 4.418e2, 5.534e2, 7.712e2, 1.088e3, 1.599e3,
      2.095e3,  3.998e3, 4.920e3, 6.724e3, 9.360e3, 1.362e4, 1.776e4, 3.343e4,
      4.084e4,  5.495e4, 7.459e4, 1.040e5, 1.302e5, 2.129e5, 2.453e5, 2.990e5,
      3.616e5,  4.384e5, 4.957e5, 6.400e5, 6.877e5, 7.603e5, 8.379e5, 9.264e5,
      9.894e5,  1.141e6, 1.189e6};
