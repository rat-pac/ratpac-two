...
void FitDirectionCenter::SetD(std::string param, double value) {
  if(param == "time_resid_low") {
    if( !fCutMethod.empty() && fCutMethod!="time" )  throw ParamInvalid(param, "Cannot specify more than one cut method for time residuals.");
    fCutMethod = "time";
    fTimeResLow = value;
    ...
    if( !fCutMethod.empty() && fCutMethod!="fraction" )  throw ParamInvalid(param, "Cannot specify more than one cut method for time residuals.");
    fCutMethod = "fraction";
    ...
    if( fLightSpeed<=0 || fLightSpeed>299.792458 )  throw ParamInvalid(param, "light_speed must be positive and <= 299.792458 mm/ns.");
    fLightSpeed = value;
  }
...
Processor::Result FitDirectionCenter::Event(DS::Root *ds, DS::EV *ev) {
  ...
  if( fPosMethod != "fixed" ) {
    ...
    if( fits.size() == 0 )
      Log::Die("FidDirectionCenter: No position specified (fixed or reconstructed).");
  ...
  if( applyDrive ) {
    std::vector<RAT::DS::FitResult *> fits = ev->GetFitResults();
    if( fits.size() == 0 )
      Log::Die("FidDirectionCenter: No reconstructed direction available for drive correction.");
    ...
    Log::Die("FidDirectionCenter: No drive value specified while direction fitter \'" + fPosFitter + "\' is specified.");
  }
  ...
  if( fCutMethod=="fraction" ) {
    std::vector<double> pmtTimes;  pmtTimes.reserve(numPMTs);
