...
inline void fourhitgrid::print_ranges(short int &hit, int &n3comb) {
  ...
  for (hit = 0; hit < nsel - 3; hit++) {
    ...
    printf("(%8.2f-%8.2f): %8.2fns ", times[hit], times[end[hit]], times[end[hit]] - times[hit]);
    // calculate and print out number of comb. with this starting hit
    n3comb = end[hit] - hit;
    printf("%3d hits, %3d combinations\n", n3comb + 1, n3comb * (n3comb - 1) * (n3comb - 2) / 6);
  }
}
...
// *************************************************************
inline void fourhitgrid::set_half_range(short int &start, short int &stop, int &n3comb) {
  ...
  for (ncombo = start = 0, stop = 3; start < nsel - 3; start++) {
    ...
    for (; stop < nsel; stop++)
      if (times[stop] - times[start] > twin) break;
    ...
    if (n3comb < 1291)
      n3comb *= (n3comb - 1) * (n3comb - 2);
    else
      n3comb = 2146687710;
    n3comb /= 6;
    if (ncombo < 2147483647 - n3comb)
      ncombo += n3comb;
    else
      ncombo = 2147483647;
  }
...
// *************************************************************
inline void fourhitgrid::adjust_range(short int &hit, short int &not_expanded, int &n3comb) {
  ...
  for (ncombo = hit = 0; hit < nsel - 3; hit++) {
    ...
    if (not_expanded)
      while ((end[hit] > hit + 3) && (times[end[hit]] - times[hit] > twin)) end[hit]--;
...
inline void fourhitgrid::find_ranges(short int nreq) {
  ...
  if (nsel < 478) {
    ...
    ncombo *= mindev;
    if (ncombo < nreq) {
      for (start = 0; start < nsel - 3; start++) end[start] = nsel - 1;
      return;
    }
  } else
    ncombo = 2147483647;
  ...
  while (thigh - tlow > 0.1) {
    ...
    if (mindev > 0.5 * nreq)
      ...
      if ((twin < tlow + 0.05) || (twin > thigh - 0.05)) twin = 0.5 * (tlow + thigh);
    }
...
// * compute all four-hit combination within end[start] range  *
// *************************************************************
inline void fourhitgrid::fourcombo(hitsel *event_hits, float tsig, float cyl_radius, float cyl_height, float dwallmin,
                                   float &goodn) {
  ...
  gmax = 4;
  for (nstore = 0; nstore < event_hits->nselected(); nstore++) cab[4 + nstore] = event_hits->sel(nstore);
    ...
    for (sec = start + 1; sec <= stop - 2; sec++) {
      ...
      for (thir = sec + 1; thir < stop; thir++) {
        ...
        for (four = thir + 1; four <= stop; four++) {
          ...
          // calculate the corresponding vertex and add to array
          if (gstart >= 0) {
            nsol = event_hits->hits::vertex4(cab, cab + 4, nstore, gstart, gend, tsig, cyl_radius, cyl_height, dwallmin,
                                             testpoint, gdn);
...
// *************************************************************
fourhitgrid::fourhitgrid(double r, double z, hitsel *hits) : searchgrid(r, z, dwall4hit()) {
  short int hit, n3c;
  float pos[3], rat;

  nsel = hits->nselected();
  printf("fourhitgrid::fourhitgrid debug: %d\n", nsel);
  if (nsel < 4) {
    nsel = 0;
    return;
  }
  ...
  printf("3\n");
  for (hit = 0; hit < nsel; hit++) times[hit] = hits->hittime(hits->sel(hit));
...
fourhitgrid::fourhitgrid(double r, double z, hitsel *hits, float tsig, float dwallmin, float &goodn)
...
fourhitgrid::fourhitgrid(void *buffer, double r, double z, hitsel *hits) : searchgrid((float *)buffer) {
  short int hit, n3c;
  float pos[3], rat;
