...
// * array or variable                                         *
// *************************************************************
char fit_param::parse(char *line, unsigned char length, char *key1, char *key2, char *key3, char type, void *var) {
  ...
  if ((type == ARRAY_FLOAT_TYPE) || (type == ARRAY_INT_TYPE) || (type == ARRAY_SHORT_INT_TYPE))
    for (ar = 0; ar < MAX_NUMBER; ar++) {
      nl = strlen(numbers[ar]) - 1;
      for (pos1 = 0; pos1 < length - nl; pos1++)
        if (!strncmp(line + pos1, numbers[ar] + 1, nl)) break;
      if (pos1 < length - nl) {
        var = (float *)var + ar;
        line += pos1 + nl;
        length -= pos1 + nl;
  ...
  length -= pos1;
  // check none
  // printf("MB: Made it this far: none check\n");
  // printf("MB: Type: %d\n",type);
  // printf("MB: key: %s, %s, %s\n",key1,key2,key3);
  ...
  if ((*line == '+') || (*line == '-'))
    ...
    pos1 = 0;
  // find end of number
  for (; (pos1 < length) && ((line[pos1] == '.') || ((line[pos1] >= '0') && (line[pos1] <= '9'))); pos1++)
    ;
...
// * where possible                                            *
// *************************************************************
inline void fit_param::parse2(char *line) {
  ...
  if (parse(line, length, (char *)"lusfit", (char *)"rid", (char *)"onstant", ARRAY_FLOAT_TYPE, &cgrid)) return;
  ...
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"pening", FLOAT_TYPE, &clusgtheta0)) return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"ositive", FLOAT_TYPE, &clusgthetaplus)) return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"egative", FLOAT_TYPE, &clusgthetaminus)) return;
  if (parse(line, length, (char *)"nitial", (char *)"Clusfit", (char *)"eight", FLOAT_TYPE, &clusgdirweight)) return;
  ...
  if (parse(line, length, (char *)"inal", (char *)"oodness", (char *)"ifference", FLOAT_TYPE, &lastdiff)) return;
  if ((np != -1) && (parse(line, length, (char *)"all", (char *)"nvoke", (char *)"iner", ARRAY_FLOAT_TYPE, cdwall)))
    return;
  if ((np != -1) &&
      (parse(line, length, (char *)"oodness", (char *)"ifference", (char *)"kim", ARRAY_FLOAT_TYPE, gdiff)))
    return;
  if (parse(line, length, (char *)"inal", (char *)"oodness", (char *)"raction", FLOAT_TYPE, &lastfrac)) return;
  if ((np != -1) && (parse(line, length, (char *)"oodness", (char *)"kim", (char *)"raction", ARRAY_FLOAT_TYPE, gfrac)))
    return;
  if (parse(line, length, (char *)"rid", (char *)"ime", (char *)"indow", FLOAT_TYPE, &tim0)) return;
  if ((np != -1) && (parse(line, length, (char *)"oodness", (char *)"ime", (char *)"indow", ARRAY_FLOAT_TYPE, time)))
    return;
  if ((np != -1) && (parse(line, length, (char *)"lusfit", (char *)"inimum", (char *)"adius", ARRAY_FLOAT_TYPE, rmin)))
    return;
  if ((np != -1) && (parse(line, length, (char *)"lusfit", (char *)"top", (char *)"adius", ARRAY_FLOAT_TYPE, rstop)))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"pening", (char *)"ngle", ARRAY_FLOAT_TYPE, clustheta0)) return;
  if (parse(line, length, (char *)"Clusfit", (char *)"ositive", (char *)"ngle", ARRAY_FLOAT_TYPE, clusthetaplus))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"egative", (char *)"ngle", ARRAY_FLOAT_TYPE, clusthetaminus))
    return;
  if (parse(line, length, (char *)"Clusfit", (char *)"ngle", (char *)"eight", ARRAY_FLOAT_TYPE, clusdirweight)) return;
  ...
  if (parse(line, length, (char *)"inal", (char *)"ikelihood", (char *)"ifference", FLOAT_TYPE, &blastdiff)) return;
  if (parse(line, length, (char *)"inal", (char *)"ikelihood", (char *)"raction", FLOAT_TYPE, &blastfrac)) return;
  ...
  if (parse(line, length, (char *)"oarse", (char *)"ikelihood", (char *)"raction", FLOAT_TYPE, &obgfrac)) return;
  ...
  if (parse(line, length, (char *)"ine", (char *)"ikelihood", (char *)"raction", FLOAT_TYPE, &cbgfrac)) return;
...
// *************************************************************
inline void fit_param::ang_constr(float &theta_0, float &theta_plus, float &theta_minus) {
...
fit_param::fit_param(void) {
  ...
  path += "/models/bonsai/fit_param.dat";
  std::string abs_filename = "fit_param.dat";
  printf("Default:%s\n local:%s\n", path.c_str(), abs_filename.c_str());
  // std::cout << path;
  FILE *pfile;

  printf("Trying to load in local file.\n");
  pfile = fopen(abs_filename.c_str(), "r");
  if (pfile != 0) {
    printf("BonsaiProc: Loading a local version of fit_param.dat\n");
  } else {
    printf("Could not find local fit_param.dat file, loading in default file.\n");
  ...
  for (fgets(line, 255, pfile); !feof(pfile); fgets(line, 255, pfile)) {
    // printf("MB: Line-Preparse:%s",line);
    parse2(line);
    // printf("MB: Line-Postparse:%s\n\n",line);
  }
  fclose(pfile);
  // print();
  ...
  if (cbrmin == -1) exit(1);
  printf("Done loading fit parameters from fit_param.dat.\n\n");
  print();
}
...
void fit_param::print(void) {
  ...
  if (clusgtheta0 == FIT_PARAM_NONE) {
    ...
    if (clusgthetaminus > 0)
      printf("Initial Clusfit angle positive deviation:  %8.2f deg\n",
             (acos(clusgtheta0 - sqrt(0.5 / clusgthetaminus)) - acos(clusgtheta0)) / PI180);
    else
      printf("Initial Clusfit angle positive deviation:   none\n");
    if (clusgthetaplus > 0)
      printf("Initial Clusfit angle negative deviation:  %8.2f deg\n",
             (acos(clusgtheta0) - acos(clusgtheta0 + sqrt(0.5 / clusgthetaplus))) / PI180);
    else
      printf("Initial Clusfit angle negative deviation:   none\n");
  }
  ...
  for (ar = 0; ar < np; ar++) {
    ...
    if (clusthetaminus[ar] > 0)
      printf("%8.2f deg\n", (acos(clustheta0[ar] - sqrt(0.5 / clusthetaminus[ar])) - acos(clustheta0[ar])) / PI180);
    else
      printf(" none\n");
    printf("%s Clusfit angle negative deviation:", numbers[ar]);
    printspace(9 - strlen(numbers[ar]));
    if (clusthetaplus[ar] > 0)
      printf("%8.2f deg\n", (acos(clustheta0[ar]) - acos(clustheta0[ar] + sqrt(0.5 / clusthetaplus[ar]))) / PI180);
    else
      printf(" none\n");
