...
void ariadne(float *vertex, int *nrhits, float *position, float *direction, float *goodness, float *quality,
             float *cos_scat, int *nrscat);
void cone_intersect(float *axis1, float *axis2, float *dir1, float *dir2, int &solution);
void find_largest_cluster(char *active, float *directions, int nrdir, float *clusdir, float &limit, int &max_index,
                          float &maxmag, int &nradd, int &clus_index);
void refine_cluster(char *active, float *directions, int nrdir, float *clusdir, float &limit, int &max_index,
                    float &maxmag, int &nradd, int &clus_index, float *cos_scat, int *nrscat);
void final_scan(char *active, float *directions, int nrdir, float *clusdir, float limit, int &max_index, float &maxmag,
                int &nradd, int &clus_index);
void sum_dir(float *directions, char *active, int nrdir, float *sum, int &nradd, float *direc, float cos_min);
 ...
 ----------------------------------------------------------------------*/
{
  ...
  *quality = 0.5 * (*nrhits) * (*nrhits + 1);
  ...
  for (row = 0; row < *nrhits; row++) {
    ...
    for (i = 0; i < 3; i++) vector[3 * row + i] /= distance;
  }
  ...
  for (row = 0; row < *nrhits; row++)
    for (column = row + 1; column < *nrhits; column++) {
      cone_intersect(vector + 3 * row, vector + 3 * column, directions + 4 * nrdir, directions + 4 * (nrdir + 1), sol);
      if (sol == 2) {
  ...
  find_largest_cluster(active, directions, nrdir, clusdir, limit, max_index, maxmag, nradd, clus_index);
  if (clus_index == 0) {
    delete[] (directions);
    delete[] (active);
    delete[] (clusdir);
  ...
  refine_cluster(active, directions, nrdir, clusdir, limit, max_index, maxmag, nradd, clus_index, cos_scat, nrscat);
  if (max_index == -1) {
    delete[] (directions);
    delete[] (active);
    delete[] (clusdir);
  ...
  *goodness = clusdir[4 * (max_index - 1) + 3] / (*quality);
  sum_dir(directions, active, nrdir, tempdir, nradd, clusdir + 4 * (max_index - 1), COS_CUT);
...
void printalldir_(float *directions, int *nrdir) {
  ...
  printf("%d directions\n", *nrdir);
  for (i = 0; i < *nrdir; i++)
    printf("%lf %lf %lf %lf\n", directions[4 * i], directions[4 * i + 1], directions[4 * i + 2], directions[4 * i + 3]);
}
...
void find_largest_cluster(char *active, float *directions, int nrdir, float *clusdir, float &limit, int &max_index,
  ...
  for (row = 0; row < nrdir; row++)
    if (active[row]) {
      sum_dir(directions, active, nrdir, clusdir + clus_index * 4, nradd, directions + 4 * row, COS_CUT);
      /*-------------------------------------------------------------------
...
void printdir_(float *dir) { printf("fortran: sum=%lf %lf %lf %lf\n", dir[0], dir[1], dir[2], dir[3]); }
