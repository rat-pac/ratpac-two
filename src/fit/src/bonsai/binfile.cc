...
binfile::binfile(char *name, char mode) {
  ...
  char *default_name = strcat(getenv("RATSHARE"), RAT::FitBonsaiProc::bs_likeFilename.c_str());
  if (fp == NULL) {
    printf(
        "FitBonsaiProc: Legacy mode -> Unable to a local file named %s! Loading user requested likelihood instead.\n",
        name);
    printf("FitBonsaiProc: Loading in likelihood file named -> %s \n", default_name);
    fp = fopen(default_name, allmode);
    if (fp == NULL) {
      exit(1);
    }
  }
...
binfile::binfile(FILE *f) { fp = f; }

binfile::~binfile(void) { fclose(fp); }
    ...
    for (j = 0; j < numbers[i]; j++, datp += sizes[i]) {
      ...
      switch (sizes[i]) {
        ...
        case 2:
          ...
          break;
        case 4:
          for (k = 2; k < 6; k++) datp[offset[k]] = primer[k - 2];
          break;
        case 8:
          for (k = 6; k < 14; k++) datp[offset[k]] = primer[k - 6];
          break;
      }
    }
  }
...
void binfile::write(int *sizes, int *numbers, void **starts) {
  ...
  for (i = 1, totsize = *sizes * (*numbers); i < narray; i++) totsize += sizes[i] * numbers[i];
  ...
  for (i = 0; i < narray; i++) {
    ...
    switch (sizes[i]) {
      case 1:
        for (j = 0; j < numbers[i]; j++) *buffp++ = *datp++;
        break;
      case 2:
        buffer[3 * i] += 32;
      ...
      case 4:
        ...
        for (j = 0; j < numbers[i]; j++, datp += 4)
          for (k = 2; k < 6; k++) *buffp++ = datp[offset[k]];
        break;
      case 8:
        buffer[3 * i] += 96;
        for (j = 0; j < numbers[i]; j++, datp += 8)
          for (k = 6; k < 14; k++) *buffp++ = datp[offset[k]];
        break;
    }
    if (sizes[i + 1] < 0) buffer[3 * i] = buffer[3 * i] | 0x80;
  }
  if (fwrite(buffer, 1, totsize + 3 * narray, fp) != (unsigned int)(totsize + 3 * narray)) {
