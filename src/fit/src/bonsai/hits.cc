...
inline double hits::findmin(double *mat, int *rowind, int *colind, int pass) {
  ...
  for (row = pass; row < 3; row++) {
    ...
    for (col = pass; col < 3; col++)
      if ((row != pass) || (col != pass))
        if ((test = absrat(mat[ind + colind[col]])) < min) {
  ...
  for (col = 3; (min >= 1e10) && (col < 5); col++) {
    ind = colind[col];
    for (row = pass; row < 3; row++)
      if ((test = absrat(mat[5 * rowind[row] + ind])) < min) {
        rowind[pass] = rowind[row];
        colind[pass] = ind;
        min = test;
      }
  ...
  for (row = pass + 1; row < 3; row++)
    if (rowind[pass] == rowind[row]) rowind[row] = saverow;
  for (col = pass + 1; col < 5; col++)
    if (colind[pass] == colind[col]) colind[col] = savecol;
...
// **********************************************
inline void hits::force_zero(double *mat, int *rowind, int *colind, int pass, int ind) {
...
int hits::inv35(double *mat, int *rowind, int *colind) {
  ...
  for (pass = 0; pass < 3; pass++) {
    ...
    for (col = pass + 1; col < 5; col++) mat[ind + colind[col]] *= factor;
...
void hits::qsort(short int *list, short int n, short int *first) {
  ...
  for (firstp = secondp = 0; (secondp < m) && ((firstp < half) || (nfirst != 0)); firstp++) {
    if (times[firstel] < times[second[secondp]]) {  // take element of the first sublist
...
// positions
// **********************************************
hits::hits(int ns, float *set, float *pmt_loc, int *bad_ch, int n_raw, int *cable, float *tim_raw, float *chg_raw) {
...
int hits::vertex4(int *hits, double *vert) {
  ...
  for (ii = 0; ii < 2; ii++)
    for (i = 0; i < 3; i++) vert[4 * ii + colind[i]] = mat[5 * rowind[i] + colind[ii + 3]];
  ...
  if ((j == 3) && (vert[7] > 0))
    for (i = 0; i < 3; i++) vert[i] = -(vert[3] * vert[i] + vert[i + 4]) + positions[3 * imin + i];
  else
    for (i = 0; i < 3; i++) {
      ii = colind[i];
      dist = vert[ii];
...
// find vertex from four-hit combination
// **********************************************
int hits::vertex4(int *fourcombo, int *hitlist, int listsize, int &hbegin, int &hend, float tsig, float cyl_radius,
                  float cyl_height, float dwallmin, double *vert, float *gdn) {
  ...
  for (s = 0; s < ns; s++) {
    ...
    for (h = hbegin; h <= hend; h++) {
      thishit = hitlist[h];
      if ((thishit != fourcombo[0]) && (thishit != fourcombo[1]) && (thishit != fourcombo[2]) &&
          (thishit != fourcombo[3])) {
