...
// *************************************************************
inline void bonsaifit::end_search(float *point, int indx, int indy, int indz,
                                  float rmax) {  // store best fit point at zero position
  point[0] = vertex[indx];
  point[1] = vertex[indy];
  point[2] = vertex[indz];
  if (rad < 0.3 * rmax)
...
#ifdef DEBUG
  printf("search %6.2fcm (%8.2f,%8.2f,%8.2f,%6.1f)", rad, start[0], start[1], start[2], start[STORERES]);
#ifdef DEBUG_TWO
  printf("\n");
#endif
#endif
...
#ifdef DEBUG_TWO
      printf("max outside: (%8.2f,%8.2f,%8.2f)", vertex[3 * max], vertex[3 * max + 1], vertex[3 * max + 2]);
#endif
...
#ifdef DEBUG_TWO
        printf("interpolation outside: (%8.2f,%8.2f,%8.2f)", vertex[3 * qual->ncheck()], vertex[3 * qual->ncheck() + 1],
