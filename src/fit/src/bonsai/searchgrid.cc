...
// *************************************************************
inline int searchgrid::close_to(double &dx, double &dy, double &dz, int p1, int p2min, int p2max, double d2max) {
...
short int searchgrid::add_point(void *buffer) {
  ...
  np = *ibuffer++;
  if (np < 0) return (0);
  if (npoint + np > mpoint) expand_size(npoint + np - mpoint);
  rfac = rmax / MAXSHORT;
  zfac = zmax / MAXSHORT;
...
void searchgrid::sparsify(float dmin) {
  ...
  if (nsparse == 0) {
    ...
    nsparse = 1;
  } else {  // otherwise, extend existing average point array
    short int *save_starts = set_starts;

    start = set_starts[nsparse - 1];
    set_starts = new short int[nsparse + 1];
    for (point = 0; point < nsparse; point++) set_starts[point] = save_starts[point];
    delete save_starts;
  ...
  for (point = start + 1; point < stop; point++) {  // find another point close-by?
    sec = close_to(dx, dy, dz, point, stop, npoint, dmin);
    if (sec == -1) {  // no, then copy the pont
      ...
      mult[npoint++] = 1;
    } else {  // yes, then add the point to the one found
      mult[sec] += mult[point];
  ...
  *ibuffer++ = np;
  if (set == 0)
    ...
    if ((set < 0) || (set == nsparse))
      start = 3 * set_starts[nsparse - 1];
    else
      start = 3 * set_starts[set - 1];  // any other set
  for (i = 0; i < np; i++) {
    val = MAXSHORT * points[start + 3 * i] / rmax;
    if (val < 0)
      *ibuffer++ = -((short int)(0.5 - val));
    else
      *ibuffer++ = ((short int)(0.5 + val));
