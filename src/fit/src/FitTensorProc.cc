...
cppflow::tensor FitTensorProc::CreateProjection(DS::EV *ev,
                                                DS::PMTInfo *pmtinfo,
  ...
  for (int i = 0; i < xdim; i++) {
    for (int j = 0; j < ydim; j++) {
      ...
      flatArray.push_back(std::min(std::max(static_cast<float>(0.0), red_value),
                                   static_cast<float>(1.0)));
      float green_value = green[i][j];
      green_value = isnan(green_value) ? 0 : green_value;
      flatArray.push_back(
          std::min(std::max(static_cast<float>(0.0), green_value),
                   static_cast<float>(1.0)));
    }
  }
...
TVector3 FitTensorProc::PositionFit(DS::Root *ds, DS::EV *ev) {
  ...
  cppflow::tensor input =
      CreateProjection(ev, pmtinfo, TVector3(0.0, 0.0, 0.0));

  auto output =
      (*this->positionModel)({{"serving_default_conv2d_input:0", input}},
                             {"StatefulPartitionedCall:0"})
          .at(0);
  const std::vector<float> &positionFitResult = output.get_data<float>();
  TVector3 positionVector(positionFitResult.at(0), positionFitResult.at(1),
                          positionFitResult.at(2));
  return positionVector;
}

TVector3 FitTensorProc::DirectionFit(DS::Root *ds, DS::EV *ev,
                                     TVector3 position) {
  DS::Run *run = DS::RunStore::Get()->GetRun(ds);
  DS::PMTInfo *pmtinfo = run->GetPMTInfo();
  ...
  auto output =
      (*this->directionModel)({{"serving_default_conv2d_input:0", input}},
                              {"StatefulPartitionedCall:0"})
          .at(0);
  const std::vector<float> &directionfit = output.get_data<float>();
  TVector3 fitDirection(directionfit.at(0), directionfit.at(1),
                        directionfit.at(2));
