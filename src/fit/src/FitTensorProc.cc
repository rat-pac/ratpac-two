...
TVector3 FitTensorProc::PositionFit(DS::Root *ds, DS::EV* ev) {
  ...
  auto output = (*this->positionModel)({{"serving_default_conv2d_input:0",input}},{"StatefulPartitionedCall:0"}).at(0);
...
TVector3 FitTensorProc::DirectionFit(DS::Root *ds, DS::EV* ev, TVector3 position) {
  ...
  auto output = (*this->directionModel)({{"serving_default_conv2d_input:0",input}},{"StatefulPartitionedCall:0"}).at(0);
