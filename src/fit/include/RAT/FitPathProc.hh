...
class FitPathProc : public Processor,
                    public Minimizable,
...
protected:
  ...
  double fPosSigma0, fPosSigma1, fThetaSigma, fPhiSigma, fTimeSigma0,
      fTimeSigma1;
  ...
  inline double PDFDirectTime(const double tresid) const {
    ...
    if (i < 0 || i >= (int)fDirectTimeProb.size())
      return 0.0;
  ...
  inline double PDFOtherTime(const double tresid) const {
    ...
    if (i < 0 || i >= (int)fOtherTimeProb.size())
      return 0.0;
  ...
  inline double PDFCherenkovAngle(const double cosalpha) const {
    ...
    if (i < 0 || i >= (int)fCosAlphaProb.size())
      return 0.0;
    return fCosAlphaProb[i];
  }

  double FTPProbability(const double x, const double y, const double z,
