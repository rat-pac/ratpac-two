...
class plato {
 ...
 public:
  inline plato(void) { nvec = 0; }
  inline ~plato(void) {
    if (nvec > 0) delete vec;
  }
  inline int ndir(void) { return (nvec); }
  inline void get_vectors(double *v) {
    int i;

    for (i = 0; i < 3 * nvec; i++) v[i] = vec[i];
  }
  inline void get_vectors(double *v, double n1, double n2, double n3) {
    ...
    for (i = 0; i < 3 * nvec; i += 3) {
      for (v[i] = vec[i] * u[0], j = 1; j < 3; j++) v[i] += vec[i + j] * u[j];
      for (v[i + 1] = vec[i] * u[3], j = 1; j < 3; j++) v[i + 1] += vec[i + j] * u[3 + j];
      for (v[i + 2] = vec[i] * u[6], j = 1; j < 3; j++) v[i + 2] += vec[i + j] * u[6 + j];
    }
  }
