...
class fourhitgrid : public fit_param, public searchgrid {
  ...
  inline void set_half_range(short int &start, short int &stop, int &n3comb);
  // calculate number of combinations if the window has changed
  inline void adjust_range(short int &hit, short int &not_expanded, int &n3comb);
  ...
  inline void fourcombo(hitsel *hits, float tsig, float cyl_radius, float cyl_height, float dwallmin, float &goodn);
