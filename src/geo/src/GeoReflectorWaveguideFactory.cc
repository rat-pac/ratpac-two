...
G4VPhysicalVolume *GeoReflectorWaveguideFactory::Construct(DBLinkPtr table) {
  ...
  if (!DB::ParseTableName(waveguide_desc, waveguide_table, waveguide_index))
    Log::Die("PMTFactoryBase: Waveguide descriptor name is not a valid RATDB "
             "table: " +
             waveguide_desc);
  ...
  for (int pmtID = 0; pmtID < max_pmts; pmtID++) {
    ...
    ball_whole = new G4SubtractionSolid(name_whole, ball_whole, waveBody,
                                        &pmtrot, waveloc);
    ball_in =
        new G4SubtractionSolid(name_in, ball_in, waveBody, &pmtrot, waveloc);
    ball_out =
        new G4SubtractionSolid(name_out, ball_out, waveBody, &pmtrot, waveloc);
  }
