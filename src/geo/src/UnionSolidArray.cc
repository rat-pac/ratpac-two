...
G4VSolid *MakeUnionSolidArray(const std::string &name, const std::vector<G4VSolid *> &solids, int firstIndex,
                              int length) {
  if (length == -1) length = solids.size();
  ...
  if (length == 0)
    ...
    G4VSolid *left = MakeUnionSolidArray(name + "L", solids, firstIndex, splitLength);
