...
G4VPhysicalVolume *GeoReflectorFactory::Construct(DBLinkPtr table) {
  ...
  G4VSolid *ball_in =
      new G4Sphere(volume_name + "_insolid", r_min, r_mid, phi_start, phi_delta,
                   theta_start, theta_delta);

  G4VSolid *ball_out =
      new G4Sphere(volume_name + "_outsolid", r_mid, r_max, phi_start,
                   phi_delta, theta_start, theta_delta);

  G4VSolid *ball_whole =
      new G4Sphere(volume_name + "_wholesolid", r_min, r_max, phi_start,
                   phi_delta, theta_start, theta_delta);
  ...
  for (int pmtID = 0; pmtID < max_pmts; pmtID++) {
    ...
    dimples_whole[pmtID] =
        new G4DisplacedSolid(name_whole, pmtBody, &pmtrot, pmtloc[pmtID]);
    dimples_in[pmtID] =
        new G4DisplacedSolid(name_in, pmtBody, &pmtrot, pmtloc[pmtID]);
    dimples_out[pmtID] =
        new G4DisplacedSolid(name_out, pmtBody, &pmtrot, pmtloc[pmtID]);
  }
  ball_in = new G4SubtractionSolid(
      ball_in->GetName() + "_sub", ball_in,
      MakeUnionSolidArray(ball_in->GetName() + "_union", dimples_in));
  ball_out = new G4SubtractionSolid(
      ball_out->GetName() + "_sub", ball_out,
      MakeUnionSolidArray(ball_out->GetName() + "_union", dimples_out));
  ball_whole = new G4SubtractionSolid(
      ball_whole->GetName() + "_sub", ball_whole,
      MakeUnionSolidArray(ball_whole->GetName() + "_union", dimples_whole));

  G4LogicalVolume *whole_logi =
      new G4LogicalVolume(ball_whole, outMaterial, volume_name);
  G4LogicalVolume *in_logi =
      new G4LogicalVolume(ball_in, inMaterial, volume_name + "_in_logi");
  G4LogicalVolume *out_logi =
      new G4LogicalVolume(ball_out, outMaterial, volume_name + "_out_logi");
  ...
  G4PVPlacement *out_phys =
      new G4PVPlacement(0,         // no rotation
                        ...
                        false,                // no boolean ops
                        0);                   // copy number
  new G4LogicalBorderSurface(volume_name + "_surface1", in_phys, out_phys,
                             RefSurface);
  new G4LogicalBorderSurface(volume_name + "_surface2", out_phys, in_phys,
                             RefSurface);
