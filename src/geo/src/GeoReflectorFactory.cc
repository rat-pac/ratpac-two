...
G4VPhysicalVolume *GeoReflectorFactory::Construct(DBLinkPtr table) {
  ...
  if (surface == "none" || surface == "")
    ...
    if (!RefSurface) Log::Die("GeoReflectorFactory: Unable to locate surface " + surface);
  }

  G4VSolid *ball_in =
      new G4Sphere(volume_name + "_insolid", r_min, r_mid, phi_start, phi_delta, theta_start, theta_delta);

  G4VSolid *ball_out =
      new G4Sphere(volume_name + "_outsolid", r_mid, r_max, phi_start, phi_delta, theta_start, theta_delta);
  ...
  G4LogicalVolume *whole_logi = new G4LogicalVolume(ball_whole, outMaterial, volume_name);
  G4LogicalVolume *in_logi = new G4LogicalVolume(ball_in, inMaterial, volume_name + "_in_logi");
  G4LogicalVolume *out_logi = new G4LogicalVolume(ball_out, outMaterial, volume_name + "_out_logi");
  ...
  G4PVPlacement *out_phys = new G4PVPlacement(0,                     // no rotation
                                              ...
                                              false,                 // no boolean ops
                                              0);                    // copy number
  new G4LogicalBorderSurface(volume_name + "_surface1", in_phys, out_phys, RefSurface);
  new G4LogicalBorderSurface(volume_name + "_surface2", out_phys, in_phys, RefSurface);
