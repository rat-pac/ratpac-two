...
G4VPhysicalVolume* GeoCherenkovSourceFactory::Construct(DBLinkPtr table) {
    ...
    //Decay chamber
    G4Material *decayChamberMaterial = G4Material::GetMaterial(table->GetS("decay_chamber_material")); //inside the decay chamber
    ...
    //Constructs the sleeveSolid - aluminium sleeve in the neck - to be placed in world
    ...
    G4Tubs *windowKnockout = new G4Tubs(volumeName+"WindowKnockout",0,pmtRadius,(knockoutThickness)/2.0,0,twopi); //leave room for the pmt
    G4ThreeVector windowKnockoutPos(0.0,0.0,(windowOd1Len+windowOd2Len)/2.0-knockoutThickness/2.0);
    G4SubtractionSolid *windowSolid = new G4SubtractionSolid(volumeName+"WindowSolid",windowFilled,windowKnockout,NULL,windowKnockoutPos);
    ...
    G4Tubs *pipeSolid = new G4Tubs(volumeName+"Tube",tubeId/2.0,tubeOd/2.0,tubeLength/2.0,0,twopi); //to be placed inside tube
    ...
    G4Tubs *sourceOuterNeck = new G4Tubs(volumeName+"SourceOuterNeck",canOd1/2.0,neckOd/2.0,neckOdLen/2.0,0,twopi);
    ...
    G4UnionSolid *sourceNeck = new G4UnionSolid(volumeName+"SourceNeck",sourceInnerNeck,sourceOuterNeck,NULL,outerOffset);
    ...
    G4SubtractionSolid *sourceFilledTubes = new G4SubtractionSolid(volumeName+"SourceFilledTubes",sourceFilledNeck,sleeveSolid,NULL,neckPos); //remove PMT volume
    G4SubtractionSolid *sourceFilledOutlet = new G4SubtractionSolid(volumeName+"SourceFilledOutlet",sourceFilledTubes,tubeSolid,inletRot,inletPos); //remove inlet volume
    G4SubtractionSolid *sourceSolid = new G4SubtractionSolid(volumeName+"SourceSolid",sourceFilledOutlet,tubeSolid,outletRot,outletPos); //remove outlet volume
    ...
    G4SubtractionSolid *blackPaintTubes = new G4SubtractionSolid(volumeName+"blackPaintTubes",paintOverlap,sleeveSolid,NULL,neckPos); //remove PMT volume
    G4SubtractionSolid *blackPaintOutlet = new G4SubtractionSolid(volumeName+"blackPaintOutlet",blackPaintTubes,tubeSolid,inletRot,inletPos); //remove inlet volume
    G4SubtractionSolid *blackPaintSolid = new G4SubtractionSolid(volumeName+"blackPaintSolid",blackPaintOutlet,tubeSolid,outletRot,outletPos); //remove outlet volume
    ...
    G4SubtractionSolid *whitePaintTubes = new G4SubtractionSolid(volumeName+"whitePaintTubes",tpbOverlap,sleeveSolid,NULL,neckPos); //remove PMT volume
    G4SubtractionSolid *whitePaintOutlet = new G4SubtractionSolid(volumeName+"whitePaintOutlet",whitePaintTubes,tubeSolid,inletRot,inletPos); //remove inlet volume
    G4SubtractionSolid *whitePaintSolid = new G4SubtractionSolid(volumeName+"whitePaintSolid",whitePaintOutlet,tubeSolid,outletRot,outletPos); //remove outlet volume

    //Constructs the decayChamberSolid - this is the He+Li8 - to be placed in whitePaintSolid
    G4Orb *decayChamberOverlap = new G4Orb(volumeName+"DecayChamberOverlap",ballId/2.0-blackPaintThickness-whitePaintThickness);
    G4SubtractionSolid *decayChamberTubes = new G4SubtractionSolid(volumeName+"DecayChamberTubes",decayChamberOverlap,sleeveSolid,NULL,neckPos); //remove PMT volume
    G4SubtractionSolid *decayChamberOutlet = new G4SubtractionSolid(volumeName+"DecayChamberOutlet",decayChamberTubes,tubeSolid,inletRot,inletPos); //remove inlet volume
    G4SubtractionSolid *decayChamberSolid = new G4SubtractionSolid(volumeName+"DecayChamberSolid",decayChamberOutlet,tubeSolid,outletRot,outletPos); //remove outlet volume

    //Construct the can solid... carefully (+1.0 fudge factors included where necessary) - to be placed in world
    ...
    //remove outer 3
    G4Tubs *canRmO3 = new G4Tubs(volumeName+"CanRmO3",canOd3/2.0,canOd1/2.0+1.0,canOd3Len/2.0+1.0,0,twopi);
    G4ThreeVector rmO3Pos(0,0,canOd1Len+canOd2Len+canOd3Len/2.0-canHalflen);
    G4SubtractionSolid *canSans2Sans3 = new G4SubtractionSolid(volumeName+"CanSans2Sans3",canSans2,canRmO3,NULL,rmO3Pos);
    //remove outer 4
    G4Tubs *canRmO4 = new G4Tubs(volumeName+"CanRmO4",canOd4/2.0,canOd1/2.0+1.0,canOd4Len/2.0+1.0,0,twopi);
    G4ThreeVector rmO4Pos(0,0,canHalflen-canOd4Len/2.0+1.0);
    G4SubtractionSolid *canFilled = new G4SubtractionSolid(volumeName+"CanFilled",canSans2Sans3,canRmO4,NULL,rmO4Pos);
    //remove inner 1
    ...
    G4SubtractionSolid *canFilledSans1 = new G4SubtractionSolid(volumeName+"CanFilledSans1",canFilled,canRmI1,NULL,rmI1Pos);
    //remove inner 2
    ...
    G4SubtractionSolid *canFilledSans1Sans2 = new G4SubtractionSolid(volumeName+"CanFilledSans1Sans2",canFilledSans1,canRmI2,NULL,rmI2Pos);
    //remove inner 3
    ...
    G4SubtractionSolid *canFilledSans1Sans2Sans3 = new G4SubtractionSolid(volumeName+"CanFilledSans1Sans2Sans3",canFilledSans1Sans2,canRmI3,NULL,rmI3Pos);
    //remove inner 4
    ...
    G4SubtractionSolid *canSolid = new G4SubtractionSolid(volumeName+"CanFilledSans1Sans2Sans3",canFilledSans1Sans2Sans3,canRmI4,NULL,rmI4Pos);
    ...
    G4LogicalVolume *decayChamberLog = new G4LogicalVolume(decayChamberSolid, decayChamberMaterial, volumeName+"_DecayChamber");

    //Assemble the PMT
//    Initialise(volumeName, windowMaterial); //FIXME - what is this material for
...
//    string pmtName = volumeName + "_pmtenv_" + ::to_string(pmtLcn); //Necessary naming for PMTOpticalModel
//
//    //Calculate where to put and how to place the PMT
    G4ThreeVector pmtPos(0,0,pmtHeight/2.0-neckIdLen/2.0+windowThickness);
    G4ThreeVector pmtDir(0.0, 0.0, 1.0); //N.B. convention is pointing the end opposite the sensitive end
...
//    double xAngle = atan2(pmtDir.y(), sqrt(pmtDir.x() * pmtDir.x() + pmtDir.z() * pmtDir.z()));
//    G4RotationMatrix* pmtRot = new G4RotationMatrix();
//    pmtRot->rotateY(yAngle);
//    pmtRot->rotateX(xAngle);
//    pmtRot->rotateZ(0.0);
//
//    //Set visualization properties
    ...
    //G4VPhysicalVolume *canPhys = ConstructPhysicalVolume(canLog, motherLog, table); 
    ...
    new G4PVPlacement(rotation, canPos+position, volumeName+"_Can", canLog, motherPhys, false, 0); //can placed in mother
//
    G4VPhysicalVolume *sleevePhys = new G4PVPlacement(rotation, neckPos+position, volumeName+"_Sleeve", sleeveLog, motherPhys, false, 0); //housing placed in mother
    /*G4VPhysicalVolume *windowPhys =*/ new G4PVPlacement(NULL, windowPos, volumeName+"_Window", windowLog, sleevePhys, false, 0); //window placed in light shield
    //Here is the code to place the PMT in the volume with the right copy number.
    //fEnvelope[0]->GetLogicalVolume() is a bit odd, but gets the envelope logical
    //volume for the previously constructed PMT from the GeoPMTBuilderBase super class.
//    /*G4VPhysicalVolume *pmtPhys =*/ new G4PVPlacement(pmtRot, pmtPos, pmtName, fEnvelope[0]->GetLogicalVolume(), sleevePhys, false, pmtLcn); //pmt placed in housing
//
    G4VPhysicalVolume *inletTubePhys = new G4PVPlacement(inletRot, inletPos, volumeName+"_InletTube", inletTubeLog, motherPhys, false, 0); //placed in mother
    /*G4VPhysicalVolume *inletPipePhys =*/ new G4PVPlacement(NULL, zero, volumeName+"_InletPipe", inletPipeLog, inletTubePhys, false, 0); //placed in inletTube
    G4VPhysicalVolume *outletTubePhys = new G4PVPlacement(outletRot, outletPos, volumeName+"_OutletTube", outletTubeLog, motherPhys, false, 0); //placed in mother
    /*G4VPhysicalVolume *outletPipePhys =*/ new G4PVPlacement(NULL, zero, volumeName+"_OutletPipe", outletPipeLog, outletTubePhys, false, 0); //placed in outletTube

    G4VPhysicalVolume *sourcePhys = new G4PVPlacement(rotation, position, volumeName+"_Acrylic", sourceLog, motherPhys, false, 0); //placed in mother
    G4VPhysicalVolume *blackPaintPhys = new G4PVPlacement(NULL, zero, volumeName+"_BlackPaint", blackPaintLog, sourcePhys, false, 0); //placed inside source
    G4VPhysicalVolume *whitePaintPhys = new G4PVPlacement(NULL, zero, volumeName+"_WhitePaint", whitePaintLog, blackPaintPhys, false, 0); //placed inside black paint
    G4VPhysicalVolume *decayChamberPhys = new G4PVPlacement(NULL, zero, volumeName+"_DecayChamber", decayChamberLog, whitePaintPhys, false, 0); //placed inside white paint
