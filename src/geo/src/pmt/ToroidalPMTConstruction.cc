...
ToroidalPMTConstruction::ToroidalPMTConstruction(DBLinkPtr table,
...
G4LogicalVolume *
ToroidalPMTConstruction::BuildVolume(const std::string &prefix) {
  if (log_pmt)
    return log_pmt;
...
void ToroidalPMTConstruction::CalcInnerParams(GLG4TorusStack *body,
  ...
  if (equatorIndex < 0)
    Log::Die("ToroidalPMTConstruction::CalcInnerParams: Pathological PMT shape "
             "with no equator edge");
  // sanity check on dynode height
  if (fParams.dynodeTop > innerZEdge[equatorIndex])
    Log::Die("ToroidalPMTConstruction::CalcInnerParams: Top of PMT dynode "
