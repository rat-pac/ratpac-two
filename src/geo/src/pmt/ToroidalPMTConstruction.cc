...
G4LogicalVolume *ToroidalPMTConstruction::BuildVolume(const std::string &prefix) {
  ...
  if (fParams.useEnvelope) envelope_solid = NewEnvelopeSolid(prefix + "_envelope_solid");
  ...
  if (fParams.useEnvelope) {
    ...
    body_phys = new G4PVPlacement(0,                      // no rotation
                                  noTranslation,          // Bounding envelope already constructed to put equator
                                                          // at origin
...
G4VSolid *ToroidalPMTConstruction::NewEnvelopeSolid(const std::string &_name) {
  ...
  for (unsigned i = 0; i < fParams.rhoEdge.size(); i++)
    if (fParams.rhoEdge[i] > rho) rho = fParams.rhoEdge[i];
...
void ToroidalPMTConstruction::CalcInnerParams(GLG4TorusStack *body, std::vector<double> &innerZEdge,
  ...
  for (int i = 1; i < nEdge; i++) {
    ...
    if (innerRhoEdge[i] > dynodeRadius && innerZEdge[i] < zLowestDynode) zLowestDynode = innerZEdge[i];
    if (outerZEdge[i] == 0.0 || innerZEdge[i] == 0.0) equatorIndex = i;
  }
  ...
  if (innerRhoEdge[nEdge] > dynodeRadius && innerZEdge[nEdge] < zLowestDynode) zLowestDynode = innerZEdge[nEdge];

  // sanity check equator index
  if (equatorIndex < 0)
    Log::Die(
        "ToroidalPMTConstruction::CalcInnerParams: Pathological PMT shape "
        "with no equator edge");
  // sanity check on dynode height
  if (fParams.dynodeTop > innerZEdge[equatorIndex])
    Log::Die(
        "ToroidalPMTConstruction::CalcInnerParams: Top of PMT dynode "
