...
LAPPDConstruction::LAPPDConstruction(DBLinkPtr table, G4LogicalVolume *mother)
    : PMTConstruction("lappd") {
  ...
  Log::Assert(fParams.width > 0,
              "LAPPDConstruction: " + pmt_model + " width must be postive");
  Log::Assert(fParams.thick > 0,
              "LAPPDConstruction: " + pmt_model + " thick must be postive");
  Log::Assert(fParams.glassThickness > 0,
              "LAPPDConstruction: " + pmt_model +
                  " glass thickness must be postive");
  Log::Assert(fParams.glass, "LAPPDConstruction: " + pmt_model +
                                 " has an invalid glass material");
  Log::Assert(fParams.vacuum, "LAPPDConstruction: " + pmt_model +
...
G4LogicalVolume *LAPPDConstruction::BuildVolume(const std::string &prefix) {
  ...
  G4Box *vacuum_solid = new G4Box(prefix + "_vacuum_solid", vacuumWidth,
                                  vacuumWidth, vacuumThick);

  // ------------ Logical Volumes -------------
  G4LogicalVolume *body_log, *vacuum_log;

  body_log =
      new G4LogicalVolume(body_solid, fParams.glass, prefix + "_body_logic");
...
G4PVPlacement *LAPPDConstruction::PlacePMT(G4RotationMatrix *pmtrot,
