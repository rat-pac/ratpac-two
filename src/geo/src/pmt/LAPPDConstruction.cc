...
G4LogicalVolume *LAPPDConstruction::BuildVolume(const std::string &prefix) {
    ...
    vacuum_phys = new G4PVPlacement(0, // no rotation
       noTranslation,            // must share the same origin than the mother volume if we want the PMT optical model working properly
...
G4PVPlacement *LAPPDConstruction::PlacePMT(G4RotationMatrix *pmtrot, G4ThreeVector pmtpos, const std::string &name,
        G4LogicalVolume *logi_pmt, G4VPhysicalVolume *mother_phys, bool booleanSolid, int copyNo) {
