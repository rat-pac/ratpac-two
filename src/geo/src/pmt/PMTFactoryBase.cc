...
G4VPhysicalVolume *PMTFactoryBase::ConstructPMTs(
  ...
  if (BFieldOn) {
    ...
    if (BFieldTableName == "" || BEffiTableName == "") {
      ...
      std::string BFieldTableName1 = static_cast<std::string>(getenv("RATSHARE")) + "/ratdb/" + ExpSubdir + "/" +
    ...
    if (BFieldOn) {
      ...
      if (jmin < 0) {
        ...
        if (dynorient.mag() == 0) {
          ...
          if (dynorient.mag() == 0) {
            info << "Warning: tried 100 times to generate a random dynode orientation for " << id
                 << "-th PMT and failed. dynorient " << dynorient(0) << "," << dynorient(1) << "," << dynorient(2)
                 << newline;
          }
        }
