...
G4VPhysicalVolume *PMTFactoryBase::ConstructPMTs(
    DBLinkPtr table, const std::vector<G4ThreeVector> &pmt_pos,
  ...
  if (mother == 0)
    Log::Die("PMTParser: Unable to find mother volume " + mother_name +
             " for " + volume_name);
  ...
  PMTConstruction *construction =
      PMTConstruction::NewConstruction(lpmt, mother);
  ...
  if (BFieldOn) {
    ...
    if (BFieldTableName == "" || BEffiTableName == "") {
      ...
      std::string ExpSubdir =
          DB::Get()->GetLink("DETECTOR")->GetS("experiment");
      std::string BFieldTableName1 =
          static_cast<std::string>(getenv("RATSHARE")) + "/ratdb/" + ExpSubdir +
      ...
      dynorfilename = static_cast<std::string>(getenv("RATSHARE")) + "/ratdb/" +
                      ExpSubdir + "/" + dynorfilename;
  ...
  for (size_t i = 0, id = pmtinfo.GetPMTCount(); i < pmt_pos.size();
       i++, id++) {

    std::string pmtname = volume_name + "_pmtenv_" +
                          std::to_string(id); // internally PMTs are represented
    ...
    pmtinfo.AddPMT(TVector3(pmtpos.x(), pmtpos.y(), pmtpos.z()),
                   ...
                   pmt_model, individual_noise_rate[i]);
    ...
    construction->PlacePMT(pmtrot, pmtpos, pmtname, log_pmt, phys_mother, false,
                           id);
  ...
  if (fastsim_log_pmt->GetFastSimulationManager() == NULL) {
    fastsim_log_pmt =
        log_pmt->GetDaughter(0)->GetLogicalVolume(); // Get the glass region
  }

  for (size_t i = 0; i < fastsim_log_pmt->GetFastSimulationManager()
                             ->GetFastSimulationModelList()
                             .size();
       i++) {
    if (fastsim_log_pmt->GetFastSimulationManager()
            ->GetFastSimulationModelList()[i]
            ->GetName() == modname) {
      ((GLG4PMTOpticalModel *)fastsim_log_pmt->GetFastSimulationManager()
           ->GetFastSimulationModelList()[i])
