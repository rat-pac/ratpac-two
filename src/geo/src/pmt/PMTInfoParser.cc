...
PMTInfoParser::PMTInfoParser(DBLinkPtr lpos_table, const std::string &mother_name) {
  ...
  try {
    const std::vector<double> &dir_x = lpos_table->GetDArray("dir_x");
    const std::vector<double> &dir_y = lpos_table->GetDArray("dir_y");
    const std::vector<double> &dir_z = lpos_table->GetDArray("dir_z");
  ...
  for (std::string parent_name = mother_name; parent_name != "";) {
