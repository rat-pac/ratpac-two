...
PMTConcentrator::PMTConcentrator(const G4String &name, G4double _upper_radius,
                                 G4double _lower_radius,
                                 G4double _a, // SEE Mike Lay's Thesis
                                 G4double _b, G4double _d,
  ...
  for (int i = 0; i < n; i++) {
    z[i] = z_from_r(lower_radius) +
           i * (z_from_r(upper_radius) - z_from_r(lower_radius)) / n;
  ...
  G4Polycone *outer_solid =
      new G4Polycone(name, 0, CLHEP::twopi, n, z, r_inner_oil, r_outer_plastic);
  SetSolid(outer_solid);

  // Split into plastic concentrator and oil layer so we can add reflective
  // surface in between
  G4Polycone *conc_solid =
      new G4Polycone(name + "_plastic", 0, CLHEP::twopi, n, z,
                     r_oil_plastic_boundary, r_outer_plastic);

  conc_logi = new G4LogicalVolume(conc_solid, bulkMaterial, name + "_plastic");

  G4Polycone *oil_solid = new G4Polycone(name + "_oil", 0, CLHEP::twopi, n, z,
                                         r_inner_oil, r_oil_plastic_boundary);
  oil_logi = new G4LogicalVolume(oil_solid, outerMaterial, name + "_oil");

  G4ThreeVector no_offset(0, 0, 0);

  G4PVPlacement *oil_phys =
  ...
  new G4LogicalBorderSurface(name + "_conc_surface1", oil_phys, conc_phys,
                             reflective_surf);
  new G4LogicalBorderSurface(name + "_conc_surface2", conc_phys, oil_phys,
                             reflective_surf);
}
