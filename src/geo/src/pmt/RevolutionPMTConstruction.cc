...
RevolutionPMTConstruction::RevolutionPMTConstruction(DBLinkPtr table,
  ...
  Log::Assert(fParams.rEdge.size() == fParams.zEdge.size(),
              "RevolutionPMTConstruction: " + pmt_model +
                  " must have equal length rho_edge, z_edge arrays");
  Log::Assert(fParams.rInner.size() == fParams.zInner.size(),
              "RevolutionPMTConstruction: " + pmt_model +
                  " must have equal length rho_inner, z_inner arrays");
  ...
  fParams.photocathode =
      Materials::optical_surface[table->GetS("photocathode_surface")];
  ...
  Log::Assert(fParams.exterior, "RevolutionPMTConstruction: " + pmt_model +
                                    " has an invalid exterior material");
  Log::Assert(fParams.glass, "RevolutionPMTConstruction: " + pmt_model +
                                 " has an invalid glass material");
  Log::Assert(fParams.vacuum, "RevolutionPMTConstruction: " + pmt_model +
                                  " has an invalid vacuum material");
  Log::Assert(fParams.dynode, "RevolutionPMTConstruction: " + pmt_model +
                                  " has an invalid dynode material");
  Log::Assert(fParams.photocathode,
              "RevolutionPMTConstruction: " + pmt_model +
                  " has an invalid photocathode material");
  Log::Assert(fParams.mirror, "RevolutionPMTConstruction: " + pmt_model +
                                  " has an invalid mirror surface");
  Log::Assert(fParams.dynode_surface,
              "RevolutionPMTConstruction: " + pmt_model +
                  " has an invalid dynode material");
...
G4LogicalVolume *
RevolutionPMTConstruction::BuildVolume(const std::string &prefix) {
  ...
  for (inner_equator_idx = 0; inner_equator_idx < fParams.zInner.size();
       inner_equator_idx++) {
    if (fParams.zInner[inner_equator_idx] <= 0.0)
      break;
  }
  size_t edge_equator_idx; // defined as zero in z
  for (edge_equator_idx = 0; edge_equator_idx < fParams.zEdge.size();
       edge_equator_idx++) {
  ...
  // inner vacuum
  G4Polycone *inner1_solid = new G4Polycone(
      prefix + "_inner1_solid", 0.0, 2.0 * CLHEP::pi, inner_equator_idx + 1,
      &fParams.zInner[0], &zeros[0], &fParams.rInner[0]);
  G4Polycone *inner2_solid =
      new G4Polycone(prefix + "_inner2_solid", 0.0, 2.0 * CLHEP::pi,
                     fParams.rInner.size() - inner_equator_idx,
                     &fParams.zInner[inner_equator_idx], &zeros[0],
                     &fParams.rInner[inner_equator_idx]);

  // dynode (FIXME solid like other tube...?)
  G4Tubs *dynode_solid =
      new G4Tubs(prefix + "_dynode_solid", 0.0, fParams.dynodeRadius,
                 fParams.dynodeHeight / 2.0, 0., CLHEP::twopi);

  // tolerance gap between inner1 and inner2, needed to prevent overlap due to
  // floating point roundoff
  G4double hhgap =
      0.5e-7; // half the needed gap between the front and back of the PMT
  G4double toleranceGapRadius =
      fParams.rInner[inner_equator_idx]; // gap goes at equator

  G4Tubs *central_gap_solid =
      new G4Tubs(prefix + "_central_gap_solid", 0.0, toleranceGapRadius, hhgap,
                 0., CLHEP::twopi);
...
G4VSolid *RevolutionPMTConstruction::BuildSolid(const string &_name) {
  ...
  G4Polycone *body =
      new G4Polycone(_name, 0, 2 * CLHEP::pi, fParams.rEdge.size(),
                     &fParams.zEdge[0], &zeros[0], &fParams.rEdge[0]);
  return body;
}
...
G4PVPlacement *RevolutionPMTConstruction::PlacePMT(
  ...
  G4PVPlacement *body_phys = new G4PVPlacement(
      pmtrot, pmtpos, _name, logi_pmt, mother_phys, booleanSolid, copyNo);
  ...
  new G4LogicalBorderSurface(_name + "_mirror_logsurf1", inner2_phys, body_phys,
                             fParams.mirror);
  new G4LogicalBorderSurface(_name + "_mirror_logsurf2", body_phys, inner2_phys,
                             fParams.mirror);

  // treate tolerance gap as mirrored also
  new G4LogicalBorderSurface(_name + "_central_gap_logsurf1", central_gap_phys,
