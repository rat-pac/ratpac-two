...
RevolutionPMTConstruction::RevolutionPMTConstruction(DBLinkPtr table, G4LogicalVolume *mother)
  ...
  Log::Assert(fParams.rEdge.size() == fParams.zEdge.size(),
              "RevolutionPMTConstruction: " + pmt_model + " must have equal length rho_edge, z_edge arrays");
  Log::Assert(fParams.rInner.size() == fParams.zInner.size(),
              "RevolutionPMTConstruction: " + pmt_model + " must have equal length rho_inner, z_inner arrays");
  ...
  Log::Assert(fParams.photocathode,
              "RevolutionPMTConstruction: " + pmt_model + " has an invalid photocathode material");
...
G4LogicalVolume *RevolutionPMTConstruction::BuildVolume(const std::string &prefix) {
  ...
  G4Polycone *inner2_solid =
      new G4Polycone(prefix + "_inner2_solid", 0.0, 2.0 * CLHEP::pi, fParams.rInner.size() - inner_equator_idx,
                     &fParams.zInner[inner_equator_idx], &zeros[0], &fParams.rInner[inner_equator_idx]);

  // dynode (FIXME solid like other tube...?)
  G4Tubs *dynode_solid =
      new G4Tubs(prefix + "_dynode_solid", 0.0, fParams.dynodeRadius, fParams.dynodeHeight / 2.0, 0., CLHEP::twopi);
  ...
  G4Tubs *central_gap_solid =
      new G4Tubs(prefix + "_central_gap_solid", 0.0, toleranceGapRadius, hhgap, 0., CLHEP::twopi);
  ...
  dynode_phys = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, fParams.dynodeTop - fParams.dynodeHeight / 2.0),
...
G4PVPlacement *RevolutionPMTConstruction::PlacePMT(G4RotationMatrix *pmtrot, G4ThreeVector pmtpos,
