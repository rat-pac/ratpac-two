...
G4VPhysicalVolume *GeoWaterBoxArrayFactory::Construct(DBLinkPtr table) {
  ...
  std::string pos_table_name = table->GetS("waterbox_locations");
  DBLinkPtr lpos_table = DB::Get()->GetLink(pos_table_name);
  const std::vector<double> &box_x = lpos_table->GetDArray("x");
  const std::vector<double> &box_y = lpos_table->GetDArray("y");
  const std::vector<double> &box_z = lpos_table->GetDArray("z");
