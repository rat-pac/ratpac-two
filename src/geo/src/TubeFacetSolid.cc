...
G4VSolid *MakeFacetSolid(const G4String &pName,
                         std::vector<G4TwoVector> polygon, G4double scale,
                         G4double hz, G4double angle, G4VSolid *basesolid) {
  ...
  for (G4int i = 0; i < Nfac; i++) {
    poly_max_tmp =
        sqrt((polygon[i].x() * CLHEP::mm * polygon[i].x() * CLHEP::mm) +
  ...
  for (G4int i = 0; i < Nfac; i++) {
    ...
    basesolid =
        new G4SubtractionSolid(pName, basesolid, cutter1, boxrot, boxtrans);
  }
  return basesolid;
}
...
G4VSolid *MakeFacetSphereSolid(const G4String &pName,
                               ...
                               std::vector<G4double> angles, G4double OR,
                               G4VSolid *basesolid) {
...
G4VSolid *MakeTubeFacetSolid(const G4String &pName,
                             std::vector<G4TwoVector> polygon, G4double scale,
                             G4double hz, G4double angle, G4double r_min,
                             G4double r_max) {
...
G4VSolid *MakeTubeFacetSolid(const G4String &pName,
                             std::vector<G4TwoVector> polygon, G4double scale,
                             G4double hz, G4double angle, G4double r_max) {
  return MakeTubeFacetSolid(pName, polygon, scale, hz, angle, 0, r_max);
}

G4VSolid *MakeConeFacetSolid(const G4String &pName,
                             std::vector<G4TwoVector> polygon, G4double scale,
                             G4double hz, G4double angle, G4double r_min_1,
                             G4double r_max_1, G4double r_min_2,
                             G4double r_max_2) {
  // G4double r_max = r_max_1;
  // if (r_max_2 > r_max_1)
  //     r_max = r_max_2;
  G4VSolid *basesolid = new G4Cons(pName, r_min_1, r_max_1, r_min_2, r_max_2,
                                   hz, 0, CLHEP::twopi);
...
G4VSolid *MakeConeFacetSolid(const G4String &pName,
                             std::vector<G4TwoVector> polygon, G4double scale,
                             G4double hz, G4double angle, G4double r_max_1,
                             G4double r_max_2) {
  return MakeConeFacetSolid(pName, polygon, scale, hz, angle, 0, r_max_1, 0,
                            r_max_2);
}

G4VSolid *MakeShellFacetSolid(const G4String &pName,
                              std::vector<G4TwoVector> polygon,
                              std::vector<G4double> angles, G4double IR,
