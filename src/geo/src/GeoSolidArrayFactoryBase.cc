...
G4VPhysicalVolume *GeoSolidArrayFactoryBase::Construct(G4VSolid *BaseSolid,
                                                       DBLinkPtr table) {
  ...
  G4LogicalVolume *logiSolid =
      new G4LogicalVolume(BaseSolid, solid_material, volume_name);
  ...
  try {
    ...
    G4VSensitiveDetector *sd =
        sdman->FindSensitiveDetector(sensitive_detector_name);
    if (sd)
...
G4VPhysicalVolume *
GeoSolidArrayFactoryBase::Construct(G4LogicalVolume *logiSolid,
                                    DBLinkPtr table) {
  string volume_name = table->GetIndex();
  ...
  for (int solidID = start_solid_num; solidID < max_solids; solidID++) {
    if ((sub_type == -1) || (sub_type == sub_type_array[solidID])) {
      ...
      // ****************************************************************
      new G4PVPlacement(solidrot, solidpos, solidname, logiSolid, phys_mother,
                        false, solidID);
