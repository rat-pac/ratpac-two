...
G4VPhysicalVolume *GeoSolidArrayFactoryBase::Construct(G4VSolid *BaseSolid, DBLinkPtr table) {
  ...
  try {
    ...
    if (invisible == 1) logiSolid->SetVisAttributes(G4VisAttributes::GetInvisible());
  } catch (DBNotFoundError &e) {
  };
  ...
  try {
    ...
    if (sd)
      ...
      Log::Die("GeoFactory error: Sensitive detector " + sensitive_detector_name + " does not exist.\n");
  } catch (DBNotFoundError &e) {
  };
...
G4VPhysicalVolume *GeoSolidArrayFactoryBase::Construct(G4LogicalVolume *logiSolid, DBLinkPtr table) {
  ...
  if (phys_mother == 0) Log::Die("GeoBuilder error: Solid mother physical volume " + mother_name + " not found");
  ...
  for (int solidID = start_solid_num; solidID < max_solids; solidID++) {
    if ((sub_type == -1) || (sub_type == sub_type_array[solidID])) {
      ...
      // rescale
      if (rescale_radius) solidpos.setMag(new_radius);
      ...
      // ****************************************************************
      new G4PVPlacement(solidrot, solidpos, solidname, logiSolid, phys_mother, false, solidID);
