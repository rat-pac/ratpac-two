...
G4VSolid *GeoLensFactory::ConstructSolid(DBLinkPtr table) {
  ...
  G4VSolid *base_lens = new G4Tubs(volume_name, 0.0, diameter / 2.0, 1.1 * thickness / 2.0, 0.0,
                                   CLHEP::twopi);  // the hole cutter

  G4VSolid *cutter_1 = new G4Sphere("temp_cutter_1", R1, 2 * R1, 0, CLHEP::twopi, 0, CLHEP::pi);

  base_lens = new G4SubtractionSolid(volume_name, base_lens, cutter_1, 0, G4ThreeVector(thickness / 2.0 - R1, 0, 0));

  G4VSolid *cutter_2 = new G4Sphere("temp_cutter_1", R2, 2 * R2, 0, CLHEP::twopi, 0, CLHEP::pi);
