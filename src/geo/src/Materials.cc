...
Materials::BuildMaterialPropertiesTable(G4Material* material, DBLinkPtr table) {
  ...
  for (vector<std::string>::iterator i=props.begin(); i!=props.end(); i++) {
    ...
    if (*i == "LIGHT_YIELD"           || *i == "dEdxCOEFF"            ||
        ...
        *i == "TOTALNUM_INT_SITES"    || *i == "MIEHG_FORWARD"        ||
        *i == "MIEHG_BACKWARD"        || *i == "MIEHG_FORWARD_RATIO"  ||
        *i == "TOTALNUM_INT_SITES"    || *i == "NUM_COMP"             ||
        *i == "SCINT_RISE_TIME"       || *i == "SCINT_RISE_TIMEalpha") {
...
void
Materials::RescaleProperty( DBLinkPtr dbTable,
                         ...
                         const std::string& property )
{
  ...
  for( size_t iScale = 0; iScale < scalings.size(); iScale++ )
    {
      stringstream propertyName; propertyName << property << iScale;
  ...
  // This is empty if no component data exists, in which case don't add the property to the properties table
  if( reciprocalSum.empty() ){
    return;
  }
    ...
    for (size_t i=0; i<components.size(); i++) {
      ...
      //// for (it=cpm->begin(); it!=cpm->end(); it++) {
        ...
        if (pname.find("REEMITWAVEFORM") != std::string::npos    ||
          ...
          Log::Assert( 
            std::find( materialPropertyNames.begin(), 
      ...
      for (int propid=0; propid < cptConstPropertyNames.size(); propid++) {
        ...
        if (cname.find("REEMISSION_PROB") != std::string::npos) {
          ...
          ////             "Materials: Composite material cannot contain the same properties as components");
          Log::Assert( 
            std::find( materialConstPropertyNames.begin(), 
