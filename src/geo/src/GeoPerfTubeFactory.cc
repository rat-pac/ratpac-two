...
G4VSolid *GeoPerfTubeFactory::ConstructSolid(DBLinkPtr table) {
  ...
  G4double r_hole = table->GetD("r_hole") *
                    CLHEP::mm; // radius of the holes.  If this is set <= zero,
  ...
  if (r_hole > 0) {
    ...
    const vector<double> &hole_r =
        lpos_table->GetDArray("r"); // radial position of hole (mm)
    const vector<double> &hole_a =
        lpos_table->GetDArray("a"); // angle of the hole (in radians)

    int num_holes = hole_a.size();

    G4VSolid *hole_cutter =
        new G4Tubs("temp", // This is the cut out piece
    ...
    for (int holeID = 0; holeID < num_holes; holeID++) {
      base_tube = new G4SubtractionSolid(
          ...
          G4ThreeVector(hole_r[holeID] * cos(hole_a[holeID]),
                        hole_r[holeID] * sin(hole_a[holeID]), 0.0));
    }
  } else {
    ...
    DBLinkPtr lpmt_model =
        DB::Get()->GetLink("PMT", lgeo_pmt->GetS("pmt_model"));
    ...
    G4ThreeVector baseloc(posvector[0] * CLHEP::mm, posvector[1] * CLHEP::mm,
                          posvector[2] * CLHEP::mm);

    for (int pmtID = 0; pmtID < max_pmts; pmtID++) {
      G4RotationMatrix pmtrot = pmt_parser.GetPMTRotation(pmtID);
      base_tube = new G4SubtractionSolid(volume_name, base_tube, pmtBody,
                                         &pmtrot, pmtloc[pmtID] - baseloc);
    }
  }
