...
void WatchmanWLSPSquareDetectorFactory::DefineDetector(DBLinkPtr /*detector*/) {
  ...
  std::vector<double> z_edge = pmt->GetDArray("z_edge");
  ...
  if (photocathode_coverage != 0.00) {  // Setting photocathode_coverage to 0 will use the baseline design
    ...
    for (int i = -rdim; i <= rdim; i++) {
      for (int j = -rdim; j <= rdim; j++) {
        if (pmt_space * sqrt(i * i + j * j) <= pmt_radius - pmt_space / 2.0) {
          topbot.push_back(make_std::pair(i, j));
        }
        if (veto_space * sqrt(i * i + j * j) <= pmt_radius - pmt_space / 2.0) {  // pmt_* is not a mistake
          topbot_veto.push_back(make_std::pair(i, j));
        }
      }
    }
    ...
    info << "Using blessed baseline design geo file.\n";

  DBLinkPtr wlsp = db->GetLink("GEO", "WLS_Plates");
  std::vector<double> pmtinfox;
  std::vector<double> pmtinfoy;
  std::vector<double> pmtinfoz;
  std::vector<double> pmtinfodirx;
  std::vector<double> pmtinfodiry;
  std::vector<double> pmtinfodirz;
  ...
  std::vector<double> dir_xp, dir_yp, dir_zp;
  ...
  if (photocathode_coverage == 0.00) {
    ...
    info << "Update geometry fields related to the reflective and absorptive "
            "tarps...\n";
    // Side tarps
  ...
  db->Set("WLSPINFO", "dir_x", dir_xp);
  db->Set("WLSPINFO", "dir_y", dir_yp);
  db->Set("WLSPINFO", "dir_z", dir_zp);
  ...
  if (photocathode_coverage != 0.00) {
    ...
    // const std::vector<double>  &cavSize = cavern->GetDArray("size_z"); //Should be
