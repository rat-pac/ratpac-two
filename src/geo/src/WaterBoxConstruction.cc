...
G4LogicalVolume *WaterBoxConstruction::ConstructLogicalVolume(DBLinkPtr table) {
  ...
  G4LogicalVolume *thebox_log = new G4LogicalVolume(thebox, cardboard, logbasename);
  ...
  G4Box *airbox = new G4Box("waterbox_air", size[0] / 2 - thickness, size[1] / 2 - thickness, size[2] / 2 - thickness);
  G4LogicalVolume *airbox_log = new G4LogicalVolume(airbox, air, logbasename + "_air");
  ...
  new G4PVPlacement((G4RotationMatrix *)0, vec, airbox_log, logbasename + "_airp", thebox_log, 0, 0);

  G4Box *water_boxpart =
      new G4Box("water_boxpart", size[0] / 2 - thickness, size[1] / 2 - thickness, size[2] / 2 - thickness);

  G4double cornervoid = table->GetD("airgapincorner");
  G4ThreeVector sz(size[0] / 2 - thickness, size[1] / 2 - thickness, size[2] / 2 - thickness);
  double_t radiuswater = sz.mag() - cornervoid;
  G4Orb *water_spherepart = new G4Orb(logbasename + "_sph", radiuswater);

  G4IntersectionSolid *water_sol = new G4IntersectionSolid("waterbox_water", water_boxpart, water_spherepart);
  G4Material *water = G4Material::GetMaterial("water");
  G4LogicalVolume *waterboxwater_log = new G4LogicalVolume(water_sol, water, logbasename + "_water");
  ...
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), waterboxwater_log, logbasename + "_waterp", airbox_log, 0, 0);
