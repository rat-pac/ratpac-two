...
ConeWaveguideConstruction::ConeWaveguideConstruction(std::string table,
                                                     std::string index) {
...
G4VSolid *ConeWaveguideConstruction::NewBodySolid(const std::string &name,
                                                  G4VSolid *fPMTBody) {
  ...
  */
  ...
  /*
    ...
    -GetPlacementOffset()); cone_solid_out->SetName(name+"_out"); cone_solid_out
    = new G4SubtractionSolid(name, cone_solid_out, fPMTBody, 0,
    -GetPlacementOffset());
  */
  G4ThreeVector NullVector(0., 0., 0.);
  cone_solid_whole->SetName(name + "_full");
  cone_solid_whole = new G4SubtractionSolid(name, cone_solid_whole, fPMTBody, 0,
                                            -GetPlacementOffset());

  return cone_solid_whole;
}

double ConeWaveguideConstruction::GetHeight() { return (fZTop - fZBottom); }
