...
void BWVetGenericChamber::Initialize(G4HCofThisEvent *HCE) {
  ...
  if (deb != 0)
    G4cout << "BWVetGenericChamber::Initialize start." << G4endl;
  _hitsCollection = new BWVetGenericChamberHitsCollection(SensitiveDetectorName,
                                                          collectionName[0]);

  if (deb != 0) {
    G4cout << "BWVetGenericChamber::Initialize hit collection address is "
           << _hitsCollection << G4endl;
  }
  if (HCID < 0) {
    HCID = G4SDManager::GetSDMpointer()->GetCollectionID(_hitsCollection);
  }
  if (deb != 0)
    G4cout << "BWVetGenericChamber::Initialize hit collection ID = " << HCID
           << G4endl;
  ...
  if (deb != 0)
    G4cout << "BWVetGenericChamber::Initialize end." << G4endl;
}

G4bool BWVetGenericChamber::ProcessHits(G4Step *aStep,
  ...
  if (deb != 0) {
    ...
    G4cout << "BWVetGenericChamber::ProcessHits getting energy deposited."
           << G4endl;
  }
  ...
  if (deb)
    G4cout << "BWVetGenericChamber::ProcessHits getting global time." << G4endl;
  ...
  if (deb != 0) {
    ...
    G4cout << "Position " << G4BestUnit(worldPos.x(), "Length") << " "
  ...
  if (deb != 0)
    G4cout << "BWVetGenericChamber::ProcessHits checking for an existing hit "
              "in this element."
           << G4endl;
  ...
  if (NULL == _hitsCollection) {
    if (deb != 0)
      G4cout << "BWVetGenericChamber::ProcessHits hit collection null. "
                "Reloading from HCofEThisEvent."
             << G4endl;
    if (_HCE) {
      _hitsCollection =
          (BWVetGenericChamberHitsCollection *)(_HCE->GetHC(HCID));
      if (deb != 0) {
        G4cout
            << "BWVetGenericChamber::ProcessHits   * hit collection address is "
            << _hitsCollection << G4endl;
      }
    } else {
      if (deb != 0)
        G4cout << "BWVetGenericChamber::ProcessHits   (E) HCofEThisEvent "
                  "pointer is NULL!"
               << G4endl;
    }
  }
  ...
  if (_hitsCollection) {
    for (G4int i = 0; i < _hitsCollection->entries(); i++) {
      ...
      // 	       << G4endl;
      if (deb != 0)
        G4cout << "  * this hit ID is " << (*_hitsCollection)[i]->GetID()
               << G4endl;
    ...
    if (ix >= 0) {
      if (deb != 0)
        G4cout << "BWVetGenericChamber::ProcessHits use existing earlier time "
                  "for hit."
               << G4endl;
    ...
    {
      if (deb != 0)
        G4cout << "BWVetGenericChamber::ProcessHits creating a new hit."
               << G4endl;
  ...
  if (deb != 0)
    G4cout << "BWVetGenericChamber::ProcessHits end." << G4endl;
  return true;
}
