/*------------------------------------------------------------------------------
  ...
  Native Windows version: switched on by macro _WIN32 which is defined by
  VC++/Borland/Mingw compilers Unix/Gnu version:   default variant, no compiler
  directives are required but _WIN32 must be absent Cygwin/Gnu version: as Unix
  version
...
////////////////////////////////////////////////////////////////////////////////
// implement string comparison of paths - Unix is case-sensitive, Windoze is
// case-insensitive
...
bool file_specification::initialise_folder(const std::string &folder_spec) {
  ...
  // first split off the drive letter or UNC prefix on Windows - the Cygwin
  // environment supports these too
  if (spec.size() >= 2 && isalpha(spec[0]) && spec[1] == ':') {
    // found a drive letter
    i = 2;
    m_drive = spec.substr(0, 2);
    m_relative = false;
    // if there is a drive but no path or a relative path, get the current path
    // for this drive and prepend it to the path
...
#endif
  // check whether the path is absolute or relative and discard the leading / if
  // absolute
  if (i < spec.size() && is_separator(spec[i])) {
    m_relative = false;
    i++;
#ifdef _WIN32
    // if there's no drive, fill it in on Windows since absolute paths must have
    // a drive
    if (m_drive.empty()) {
  ...
  // now extract the path elements - note that a trailing / is not significant
  // since /a/b/c/ === /a/b/c also note that the leading / has been discarded -
  // all paths are relative if absolute() is set, then paths are relative to the
  // drive, else they are relative to the current path
  unsigned start = i;
  while (i <= spec.size()) {
    if (i == spec.size()) {
...
bool file_specification::simplify(void) {
  // simplify the path by removing unnecessary . and .. entries - Note that
  // zero-length entries are treated like .
  for (unsigned i = 0; i < m_path.size();) {
    if (m_path[i].empty() || m_path[i].compare(".") == 0) {
      ...
      if (i == 0 && !m_relative) {
        ...
        // the first element of a relative path or the previous element is ..
        // then keep it
        i++;
      } else {
...
bool file_specification::make_relative(const file_specification &rootspec) {
  ...
  // element in root
  unsigned i = 0;
  while (subpath_size() > 0 && i < rootspec.subpath_size() &&
         path_compare(subpath_element(0), rootspec.subpath_element(i))) {
    subpath_erase(0);
    i++;
  }
...
#ifdef _WIN32
// file type tests are not defined for some reason on Windows despite them
// providing the stat() function!
#define R_OK 4
#define W_OK 2
#define X_OK 1
#define F_OK 0
...
bool is_folder(const std::string &thing) {
  ...
  spec.initialise_folder(thing);
  struct stat buf;
  if (!(stat(spec.image().c_str(), &buf) == 0)) {
    return false;
  }
...
bool file_exists(const std::string &filespec) { return is_file(filespec); }
...
std::string folder_current(void) { return "."; }

std::string folder_current_full(void) {
  // It's not clear from the documentation whether the buffer for a path should
  // be one byte longer than the maximum path length to allow for the null
  // termination, so I have made it so anyway
#ifdef _WIN32
...
static std::string full_path(const std::string &root, const std::string &path) {
  // convert path to a full path using root as the start point for relative
  // paths decompose the path and test whether it is already an absolute path,
...
std::string filename_part(const std::string &spec) {
  // scan back through filename until a preferred_separator is found and remove
  // prefix; if there is no preferred_separator then remove nothing, i.e. the
...
// 6) %path%
// currently only (2) and (6) has been implemented although many system folders
// are on the path anyway also implement the implied .exe extension on commands
// with no path (see CreateProcess documentation)
