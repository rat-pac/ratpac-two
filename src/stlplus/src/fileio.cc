...
std::string ofbuff::error_string(void) const {
  // if the number is positive, it is an errno, otherwise it is a built-in and
  // can be handled by the baseclass
  if (error_number() > 0)
...
void ofbuff::open(const std::string &fname, size_t bufsize, otext::open_t mode, bool _line_buffer) {
  ...
  if (fname.empty()) {
    ...
    // on Windoze fopen raises an assertion failure in this case which is
    // unacceptable so I trap it first
...
std::string ifbuff::error_string(void) const {
  // if the number is positiove, it is an errno, otherwise it is a built-in and
  // can be handled by the baseclass
  if (error_number() > 0)
    return std::string(strerror(error_number()));
  else
