...
assert_failed::assert_failed(const char *file, int line, const char *function,
                             const std::string &message) throw()
    : std::logic_error(std::string(file) + std::string(":") + to_string(line) +
                       std::string(":") +
                       std::string(function ? function : unknown_function) +
...
void _debug_global(const char *file, int line, const char *function,
                   bool state) {
  debug_global = state;
  fprintf(stderr, "%s:%i:[%i]%s ", filename_part(file).c_str(), line,
          debug_trace_depth, function ? function : unknown_function);
  fprintf(stderr, "debug global : %s\n", debug_global ? "on" : "off");
}

void _debug_assert_fail(const char *file, int line, const char *function,
...
_debug_trace::_debug_trace(const char *f, int l, const char *fn)
    : m_file(f), m_line(l), m_function(fn ? fn : unknown_function), m_depth(0),
      m_last(debug_last), m_dbg(false), m_old(false) {
  if (!debug_read) {
    debug_read = true;
  ...
  m_dbg = debug_set ||
          (debug_match && (!debug_match[0] ||
                           match_wildcard(debug_match, filename_part(m_file))));
  m_old = debug_set;
