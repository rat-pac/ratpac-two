...
std::string otext::error_string(void) const {
  if (!error())
    return std::string();
  return m_buffer ? m_buffer->error_string()
                  : textio_error_string(textio_uninitialised);
}

void otext::set_error(int _error) {
  if (m_buffer)
    m_buffer->set_error(_error);
}

void otext::clear_error(void) {
  if (m_buffer)
    m_buffer->clear_error();
}

void otext::set_newline_mode(otext::newline_t _newline) {
  if (m_buffer)
    m_buffer->set_newline_mode(_newline);
}
...
void otext::set_integer_width(unsigned width) {
  if (m_buffer)
    m_buffer->set_integer_width(width);
}
...
void otext::set_integer_radix(unsigned radix) {
  if (m_buffer)
    m_buffer->set_integer_radix(radix);
}
...
void otext::set_integer_display(radix_display_t display) {
  if (m_buffer)
    m_buffer->set_integer_display(display);
}
...
void otext::set_real_precision(unsigned width) {
  if (m_buffer)
    m_buffer->set_real_precision(width);
}
...
void otext::set_real_display(real_display_t display) {
  if (m_buffer)
    m_buffer->set_real_display(display);
}
...
otext &otext::operator<<(short s) {
  try {
    operator<<(
        to_string(s, integer_radix(), integer_display(), integer_width()));
...
otext &otext::operator<<(int i) {
  try {
    operator<<(
        to_string(i, integer_radix(), integer_display(), integer_width()));
  } catch (const std::invalid_argument) {
    set_error(textio_format_error);
  }
  return *this;
...
otext &otext::operator<<(long l) {
  try {
    operator<<(
        to_string(l, integer_radix(), integer_display(), integer_width()));
  } catch (const std::invalid_argument) {
    set_error(textio_format_error);
  }
  return *this;
...
void skipspaces(itext &it) {
  for (int ch = it.peek(); ch != -1 && ch != newline && isspace(ch);
       ch = it.peek())
    it.get();
}
...
obuff::obuff(bool _line_buffer, otext::newline_t _newline)
    : m_line_buffer(_line_buffer), m_newline(_newline), m_bytes(0), m_line(1),
      m_column(0), m_integer_width(0), m_integer_radix(10),
      m_integer_display(radix_c_style_or_hash), m_real_width(0),
      m_real_precision(6), m_real_display(display_mixed), m_error_number(0),
      m_aliases(0) {}
...
ibuff::ibuff(itext::newline_t _newline)
    : m_newline_mode(_newline), m_bytes(0), m_line(1), m_column(0),
      m_error_number(0), m_aliases(0) {}
