...
public:
  ...
  TrackNav(RAT::DS::MC *mc, bool verbose = false) { Load(mc, verbose); };
  void Load(RAT::DS::MC *mc, bool verbose = false);
  void Clear();

  TrackNode *Head() { return fHead; };
  TrackCursor Cursor(bool verbose = false) {
    return TrackCursor(fHead, verbose);
  };

  TrackNode *FindID(int trackID);
  TrackNode *FindParticle(const std::string &particleName,
                          bool verbose = false);
