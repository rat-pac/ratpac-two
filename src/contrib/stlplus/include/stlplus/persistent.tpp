...
template<typename T>
void dump_enum(dump_context& context, const T& data) throw()
{
  dump(context, (unsigned)data);
}

template<typename T>
void restore_enum(restore_context& context, T& data) throw()
{
  unsigned value = 0;
...
void dump_basic_string(dump_context& context, const std::basic_string<charT,traits,allocator>& data) throw()
{
...
template<typename charT, typename traits, typename allocator>
void restore_basic_string(restore_context& context, std::basic_string<charT,traits,allocator>& data) throw()
{
  data.erase();
  size_t size = 0;
...
template<typename T>
void dump_pointer(dump_context& context, const T* const data) throw()
{
...
template<typename T>
void restore_pointer(restore_context& context, T*& data) throw()
{
  if (data)
  {
    delete data;
...
template<typename T>
void dump_xref(dump_context& context, const T* const data) throw()
{
  // register the address and get the magic key for it
  std::pair<bool,unsigned> mapping = context.pointer_map(data);
  // if this is the first view of this pointer, simply throw an exception
  if (!mapping.first) throw ("tried to dump a cross-reference not seen before");
  // otherwise, just dump the magic key
  dump(context,mapping.second);
}

template<typename T>
void restore_xref(restore_context& context, T*& data) throw()
{
...
template<typename T>
void dump_polymorph(dump_context& context, const T* data) throw()
{
  try
  {
    // register the address and get the magic key for it
...
template<typename T>
void restore_polymorph(restore_context& context, T*& data) throw()
{
  try
  {
...
template<typename T>
void dump_interface(dump_context& context, const T* data) throw()
{
  try
  {
    // register the address and get the magic key for it
...
template<typename T>
void restore_interface(restore_context& context, T*& data) throw()
{
  try
  {
    // first delete any previous object pointed to since the restore creates the object of the right subclass
...
template<size_t N>
void dump_bitset(dump_context& context, const std::bitset<N>& data) throw()
{
...
template<size_t N>
void restore_bitset(restore_context& context, std::bitset<N>& data) throw()
{
  size_t bits = data.size();
  size_t bytes = (bits+7)/8;
  for (size_t B = bytes; B--; )
...
template<typename T>
void dump_complex(dump_context& context, const std::complex<T>& data) throw()
{
...
template<typename T>
void restore_complex(restore_context& context, std::complex<T>& data) throw()
{
  T re, im;
  restore(context,re);
  restore(context,im);
...
template<typename T>
void dump_deque(dump_context& context, const std::deque<T>& data) throw()
{
  dump(context,data.size());
...
template<typename T>
void restore_deque(restore_context& context, std::deque<T>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename T>
void dump_list(dump_context& context, const std::list<T>& data) throw()
{
  dump(context,data.size());
...
template<typename T>
void restore_list(restore_context& context, std::list<T>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
void dump_pair(dump_context& context, const std::pair<K,T>& data) throw()
{
...
template<typename K, typename T>
void restore_pair(restore_context& context, std::pair<K,T>& data) throw()
{
...
void dump_map(dump_context& context, const std::map<K,T,P>& data) throw()
{
  dump(context,data.size());
...
template<typename K, typename T, typename P>
void restore_map(restore_context& context, std::map<K,T,P>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename K, typename T, typename P>
void dump_multimap(dump_context& context, const std::multimap<K,T,P>& data) throw()
{
  dump(context,data.size());
...
template<typename K, typename T, typename P>
void restore_multimap(restore_context& context, std::multimap<K,T,P>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
void dump_set(dump_context& context, const std::set<K,P>& data) throw()
{
  dump(context,data.size());
...
template<typename K, typename P>
void restore_set(restore_context& context, std::set<K,P>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename K, typename P>
void dump_multiset(dump_context& context, const std::multiset<K,P>& data) throw()
{
  dump(context,data.size());
...
template<typename K, typename P>
void restore_multiset(restore_context& context, std::multiset<K,P>& data) throw()
{
  data.clear();
  size_t size = 0;
  restore(context,size);
...
template<typename T>
void dump_vector(dump_context& context, const std::vector<T>& data) throw()
{
  dump(context,data.size());
  for (size_t i = 0; i < data.size(); i++)
    dump(context,data[i]);
...
template<typename T>
void restore_vector(restore_context& context, std::vector<T>& data) throw()
{
  size_t size = 0;
  restore(context,size);
  data.resize(size);
